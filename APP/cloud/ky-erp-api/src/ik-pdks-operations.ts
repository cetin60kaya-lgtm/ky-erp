...
function companyOf(c: Context<AppEnv>, body: Row = {}) {
  return text(c.req.header("X-KYERP-Tenant-Slug") || body.mainCompanyId || body.mainCompanySlug || c.req.query("mainCompanyId") || c.req.query("mainCompanySlug") || DEFAULT_COMPANY)
...
async function authContext(c: Context<AppEnv>, body: Row = {}) {
  ...
  if (!user) return null;
  const company = text(user.mainCompanySlug || user.security?.main_company_slug || companyOf(c, body) || DEFAULT_COMPANY).toLocaleLowerCase("tr-TR");
  let audit = upper(user.role) === "DENETIM" || text(user.username).toLocaleLowerCase("tr-TR") === "denetim";
  if (!audit) {
    try {
      const row = await c.env.DB.prepare("SELECT scope FROM ik_user_hr_scope WHERE user_id=? AND main_company_id=? LIMIT 1").bind(user.id, company).first<Row>();
...
async function full(c: Context<AppEnv>, body: Row = {}) {
...
async function ensurePeriodOpen(c: Context<AppEnv>, company: string, date: string) {
  ...
  if (!period) throw new Error("Tarih geçersiz.");
  if (!(await tableExists(c, "ik_monthly_close"))) return;
  const row = await first(c, "SELECT is_locked FROM ik_monthly_close WHERE main_company_id=? AND period_year=? AND period_month=? LIMIT 1", [company, period.year, period.month]);
  if (Number(row?.is_locked || 0) !== 0) {
    const err: any = new Error(`Dönem kilitli: ${period.year}-${String(period.month).padStart(2, "0")}.`);
    err.code = "PDKS_PERIOD_LOCKED";
    throw err;
  }
}
...
async function officialHolidayRows(c: Context<AppEnv>, company: string, start = "", end = "") {
  ...
  const rows = await all(c, `SELECT data FROM json_store WHERE scope='IK_OFFICIAL_HOLIDAY' AND (main_company_slug=? OR main_company_slug IS NULL) ORDER BY updated_at DESC`, [company]);
  const result: Row[] = [];
  const seen = new Set<string>();
...
async function saveHoliday(c: Context<AppEnv>) {
  ...
  const result = await full(c, body);
  ...
  const existing = await first(c, "SELECT id FROM json_store WHERE scope='IK_OFFICIAL_HOLIDAY' AND file_name=? AND (main_company_slug=? OR main_company_slug IS NULL) LIMIT 1", [fileName, auth.company]);
  const payload = JSON.stringify({ id: fileName, date, name, halfDay: Boolean(body.halfDay), note: text(body.note), updatedAt: timestamp });
  if (existing?.id) {
    await c.env.DB.prepare("UPDATE json_store SET data=?,main_company_slug=?,updated_at=? WHERE id=?").bind(payload, auth.company, timestamp, existing.id).run();
  } else {
    await c.env.DB.prepare("INSERT INTO json_store(id,scope,main_company_slug,file_name,data,created_at,updated_at) VALUES(?,?,?,?,?,?,?)")
      .bind(crypto.randomUUID(), "IK_OFFICIAL_HOLIDAY", auth.company, fileName, payload, timestamp, timestamp).run();
  }
  return ok(c, { date, name, halfDay: Boolean(body.halfDay) }, 201);
}
...
async function leaveRows(c: Context<AppEnv>, company: string, from = "", to = "") {
  ...
  const rows = await all(c, `SELECT p.*,e.full_name,e.code FROM ik_leave_plans p
  ...
  return rows.map((row) => ({
    id: text(row.id), employeeId: text(row.employee_id), fullName: text(row.full_name), personnelCode: text(row.code),
...
async function saveLeave(c: Context<AppEnv>) {
  ...
  if (!end && returnDateInput) {
    const d = new Date(`${returnDateInput}T12:00:00Z`); d.setUTCDate(d.getUTCDate() - 1); end = d.toISOString().slice(0, 10);
  }
  if (!employeeId || !start || !end || end < start) return error(c, 400, "LEAVE_FIELDS_REQUIRED", "Personel ve geçerli izin tarihleri zorunludur.");
  const person = await employee(c, auth.company, employeeId);
  if (!person || upper(person.sgk_status) !== "VAR" || !text(person.card_no)) return error(c, 404, "PDKS_PERSON_NOT_FOUND", "SGK=VAR + kartlı personel bulunamadı.");
  ...
  const conflict = await first(c, `SELECT id FROM ik_leave_plans WHERE main_company_id=? AND employee_id=? AND status<>'CANCELLED' AND start_date<=? AND end_date>=? LIMIT 1`, [auth.company, employeeId, end, start]);
  if (conflict) return error(c, 409, "LEAVE_CONFLICT", "Personelin seçilen tarihlerde başka izin kaydı var.");
  ...
  await c.env.DB.prepare(`INSERT INTO ik_leave_plans
    ...
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(id, auth.company, employeeId, annual ? "Yıllık izin" : text(body.recordType || "İzin"), start, end, returnDate, counted.length, JSON.stringify(excluded), text(body.status || "APPROVED"), text(body.documentNo), text(body.note), text(auth.user.username || auth.user.id), nowIso(), nowIso()).run();
  await audit(c, auth.company, employeeId, start.slice(0, 7), "PDKS_IZIN_EKLE", {}, { id, start, end, countedDays: counted.length }, text(body.note), text(auth.user.username));
...
async function adjustmentRows(c: Context<AppEnv>, company: string, year: number, month: number) {
  ...
  return rows.map((row) => ({
    ...
    adjustmentType: text(row.adjustment_type), hourOrDay: number(row.hour_or_day), amount: number(row.amount), paymentMethod: text(row.payment_method || "Elden"),
...
async function saveAdvance(c: Context<AppEnv>) {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare(`INSERT INTO hr_monthly_adjustments_v2(id,employee_id,date,adjustment_type,hour_or_day,amount,payment_method,payroll_effect,note,status)
    VALUES(?,?,?,?,?,?,?,?,?,?)`)
    .bind(id, employeeId, date, "Avans", 0, amount, text(body.paymentMethod || "Elden"), text(body.payrollEffect || "BORDRO_AZALTIR"), text(body.note), text(body.status || "APPROVED")).run();
  await audit(c, auth.company, employeeId, date.slice(0, 7), "PDKS_AVANS_EKLE", {}, { id, date, amount }, text(body.note), text(auth.user.username));
  return ok(c, { id, employeeId, date, amount }, 201);
}

async function payroll(c: Context<AppEnv>) {
  const result = await full(c);
  if (result.response) return result.response;
  const { auth } = result;
  ...
  const employees = await all(c, `SELECT e.id,e.code,e.full_name,e.salary,e.bank_amount,e.cash_amount,s.card_no
    ...
    WHERE e.main_company_id=? AND UPPER(TRIM(COALESCE(e.sgk_status,'')))='VAR' AND TRIM(COALESCE(s.card_no,''))<>'' ORDER BY e.code,e.full_name`, [auth.company]);
  const adjustments = await adjustmentRows(c, auth.company, year, month);
  const byEmployee = new Map<string, Row[]>();
  for (const row of adjustments) {
    const key = text(row.employeeId); if (!byEmployee.has(key)) byEmployee.set(key, []); byEmployee.get(key)!.push(row);
  }
...
async function audit(c: Context<AppEnv>, company: string, employeeId: string, period: string, actionType: string, oldValue: unknown, newValue: unknown, reason: string, userName: string) {
  if (!(await tableExists(c, "ik_audit_logs"))) return;
  try {
    await c.env.DB.prepare(`INSERT INTO ik_audit_logs(id,main_company_id,employee_id,period,action_type,source_screen,old_json,new_json,reason,user_name,created_at)
      VALUES(?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(crypto.randomUUID(), company, employeeId || "", period || "", actionType, "KY_PDKS", JSON.stringify(oldValue || {}), JSON.stringify(newValue || {}), reason || "", userName || "", nowIso()).run();
  } catch {}
}
...
async function auditLogs(c: Context<AppEnv>) {
  ...
  const rows = await all(c, `SELECT * FROM ik_audit_logs WHERE main_company_id=? AND (?='' OR period=?) ORDER BY created_at DESC LIMIT ?`, [auth.company, period, period, limit]);
  return ok(c, rows.map((row) => ({ id: text(row.id), employeeId: text(row.employee_id), period: text(row.period), actionType: text(row.action_type), sourceScreen: text(row.source_screen), reason: text(row.reason), userName: text(row.user_name), createdAt: row.created_at })));
}
...
async function closeBlocking(c: Context<AppEnv>, company: string, year: number, month: number) {
  ...
  for (const row of events) { const key = `${text(row.employee_id)}|${dateOnly(row.work_date)}`; eventCount.set(key, (eventCount.get(key) || 0) + 1); }
  ...
  if (await tableExists(c, "ik_leave_plans")) {
    ...
    for (const leave of leaves) for (const date of eachDate(dateOnly(leave.start_date) < start ? start : dateOnly(leave.start_date), dateOnly(leave.end_date) > end ? end : dateOnly(leave.end_date))) leaveMap.add(`${text(leave.employee_id)}|${date}`);
  }
  const missing: Row[] = [];
  let checks = 0;
...
async function periodClose(c: Context<AppEnv>) {
  ...
  if (lock) {
    ...
    if (blocking.missing.length) return ok(c, { year, month, blockingCount: blocking.missing.length, okCount: Math.max(0, blocking.checks - blocking.missing.length), totalChecks: blocking.checks, isLocked: false, missing: blocking.missing.slice(0, 100) });
  }
  ...
  const timestamp = nowIso();
  await c.env.DB.prepare(`INSERT INTO ik_monthly_close(id,main_company_id,period_year,period_month,is_locked,locked_at,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(main_company_id,period_year,period_month) DO UPDATE SET is_locked=excluded.is_locked,locked_at=excluded.locked_at,updated_at=excluded.updated_at`)
  ...
  if (await tableExists(c, "ik_monthly_close_logs")) {
    try { await c.env.DB.prepare(`INSERT INTO ik_monthly_close_logs(id,main_company_id,period_year,period_month,action,reason,old_json,new_json,user_name,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`)
      .bind(crypto.randomUUID(), auth.company, year, month, lock ? "LOCK" : "UNLOCK", text(body.reason), JSON.stringify(existing || {}), JSON.stringify({ isLocked: lock }), text(auth.user.username), timestamp).run(); } catch {}
  }
  await audit(c, auth.company, "", `${year}-${String(month).padStart(2, "0")}`, lock ? "PDKS_DONEM_KAPAT" : "PDKS_DONEM_AC", existing || {}, { isLocked: lock }, text(body.reason), text(auth.user.username));
...
async function monthOverview(c: Context<AppEnv>) {
  ...
  const adjustments = await adjustmentRows(c, auth.company, year, month);
  let close: Row = {};
  if (await tableExists(c, "ik_monthly_close")) close = await first(c, "SELECT * FROM ik_monthly_close WHERE main_company_id=? AND period_year=? AND period_month=? LIMIT 1", [auth.company, year, month]) || {};
...
export function registerIkPdksOperationRoutes(app: Hono<AppEnv>) {
  ...
  app.get("/api/ik/personnel-control/operations/leaves", async (c) => {
    const result = await full(c); if (result.response) return result.response;
