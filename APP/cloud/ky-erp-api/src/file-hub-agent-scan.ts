...
const allowed=(c:Context<AppEnv>)=>Boolean(text(c.env.FILE_HUB_AGENT_KEY)&&text(c.env.FILE_HUB_AGENT_KEY)===text(c.req.header("X-KYERP-Agent-Key")));
const slugOf=(c:Context<AppEnv>,b:Row)=>text(b.mainCompanySlug||b.main_company_slug||c.req.header("X-KYERP-Tenant-Slug")||"mecit-hakan");
...
export function registerPublicFileHubScanRoutes(app:Hono<AppEnv>){
  ...
  app.post("/api/auth/file-hub-agent/scan-complete",async c=>{
    if(!allowed(c))return c.json(err("AGENT_UNAUTHORIZED","Agent anahtarı geçersiz."),401);
    const b=await bodyOf(c),slug=slugOf(c,b),connectionId=text(b.storageConnectionId),startedAt=text(b.startedAt),ts=now();
