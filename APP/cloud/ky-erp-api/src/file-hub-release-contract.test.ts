...
test("File Hub + accounting + PDKS + billing production path is explicit and targeted", () => {
  ...
  for (const migration of FINAL_MIGRATIONS) {
    const pattern = new RegExp(migration.replaceAll(".", "\\."));
    assert.match(migrate, pattern);
  ...
  assert.match(migrate, /company_billing_profiles/);
  assert.match(migrate, /file_hub_agent_credentials/);
  ...
  assert.match(preflight, /file_hub_agent_credentials/);
  assert.match(preflight, /company_billing_profiles/);
  assert.doesNotMatch(preflight, /--remote/);

  assert.match(workflow, /Canli D1 tam yedegini al/);
  assert.match(workflow, /company_billing_profiles/);
  assert.match(workflow, /company_billing_ledger/);
  assert.match(workflow, /file_hub_agent_credentials/);
  assert.match(workflow, /wrangler deploy/);
  assert.match(workflow, /wrangler@4\.118\.0 pages deploy/);
  ...
  const migrationRun = launcher.lastIndexOf("KYERP_FILE_HUB_ACCOUNTING_PRODUCTION_MIGRATE.ps1");
  const deployRun = launcher.lastIndexOf("KYERP_DIRECT_PRODUCTION.ps1");
  assert.ok(migrationRun >= 0);
  assert.ok(deployRun >= 0);
  assert.ok(migrationRun < deployRun, "targeted D1 migration must complete before Worker/Pages deploy");
});
