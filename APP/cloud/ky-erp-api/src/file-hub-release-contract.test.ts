...
test("File Hub + accounting production path is explicit and targeted", () => {
  ...
  for (const migration of ["0028_file_hub_multistorage.sql","0029_file_hub_outgoing_team_links.sql","0030_file_hub_provider_defaults.sql","0031_file_hub_primary_location_failover.sql","0032_accounting_document_core.sql","0033_accounting_intelligence_profiles.sql","0034_accounting_document_archive_queue.sql"]) { assert.match(migrate, new RegExp(migration.replaceAll(".", "\\."))); assert.match(preflight, new RegExp(migration.replaceAll(".", "\\."))); }
