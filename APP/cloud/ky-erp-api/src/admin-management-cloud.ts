...
function canonicalPermissions(role: string, source: unknown) {
  ...
  return MODULE_KEYS.map((moduleKey) => {
    ...
    return { moduleKey, canView,
      canCreate: boolValue(row.canCreate ?? row.can_create, false), canUpdate: boolValue(row.canUpdate ?? row.can_update, false),
      canDelete: boolValue(row.canDelete ?? row.can_delete, false), canApprove: boolValue(row.canApprove ?? row.can_approve, false) };
  });
}
