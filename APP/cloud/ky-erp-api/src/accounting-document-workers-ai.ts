...
Belgeyi düzenleyen/gönderen taraf supplier, belgeyi alan taraf customer alanlarına yaz. İrsaliyede supplier=gönderen, customer=teslim alan kabul edilir.
JSON şeması:
{"documentKind":"FATURA|IRSALIYE","supplierName":"","supplierTaxNo":"","customerName":"","customerTaxNo":"","documentNo":"","issueDate":"YYYY-MM-DD","dueDate":"YYYY-MM-DD veya boş","currency":"TRY","subtotal":0,"taxTotal":0,"discountTotal":0,"payableTotal":0,"confidence":0.0,"rawText":"belgedeki okunabilir ana metin","lines":[{"lineNo":1,"productCode":"","supplierProductCode":"","description":"","quantity":0,"unitCode":"","unitPrice":0,"taxRate":0,"taxAmount":0,"discountTotal":0,"lineTotal":0,"lotNo":"","confidence":0.0}]}
İstenen tür: ${kind||"AUTO"}. AUTO ise başlığa ve belge ibarelerine göre FATURA/IRSALIYE seç. LOT, PARTİ veya BATCH numarası varsa satırın lotNo alanına yaz. Vergi/VKN/TCKN yalnız belgede görünüyorsa yaz. Tutarları sayı olarak yaz.`}
function canonical(row:Row,kind:string,answer:string){
  const lines=(Array.isArray(row.lines)?row.lines:[]).slice(0,250).map((line:Row,index:number)=>({lineNo:Number(line.lineNo||index+1),productCode:text(line.productCode),supplierProductCode:text(line.supplierProductCode||line.productCode),description:text(line.description),quantity:num(line.quantity),unitCode:text(line.unitCode),unitPrice:num(line.unitPrice),taxRate:num(line.taxRate),taxAmount:num(line.taxAmount),discountTotal:num(line.discountTotal),lineTotal:num(line.lineTotal),lotNo:text(line.lotNo)||lotFrom(line.description),extractionConfidence:clamp(line.confidence||row.confidence||.65)}));
  const documentKind=/IRSALIYE|DISPATCH|DESPATCH/i.test(text(row.documentKind))?"IRSALIYE":/FATURA|INVOICE/i.test(text(row.documentKind))?"FATURA":/IRSALIYE|DISPATCH|DESPATCH/i.test(kind)?"IRSALIYE":"FATURA";
  const supplierName=text(row.supplierName||row.issuerName||row.partyName),supplierTaxNo=text(row.supplierTaxNo||row.issuerTaxNo||row.partyTaxNo).replace(/\D/g,""),customerName=text(row.customerName||row.receiverName),customerTaxNo=text(row.customerTaxNo||row.receiverTaxNo).replace(/\D/g,"");
  const required=[supplierName,supplierTaxNo,customerName,customerTaxNo,text(row.documentNo),dateOnly(row.issueDate),num(row.payableTotal)>0?"1":"",lines.length?"1":""].filter(Boolean).length;
  const confidence=Math.max(clamp(row.confidence),Math.min(.92,.42+required*.06));
  return{extractor:"CLOUDFLARE_WORKERS_AI",extractorModel:MODEL,extractionConfidence:confidence,inferredDocumentKind:documentKind,supplierName,supplierTaxNo,customerName,customerTaxNo,partyName:supplierName,partyTaxNo:supplierTaxNo,partyTaxOffice:"",partyIban:"",documentNo:text(row.documentNo),uuid:"",issueDate:dateOnly(row.issueDate),dueDate:dateOnly(row.dueDate),currency:text(row.currency)||"TRY",subtotal:num(row.subtotal),taxTotal:num(row.taxTotal),discountTotal:num(row.discountTotal),payableTotal:num(row.payableTotal),lines,rawText:text(row.rawText).slice(0,20000),rawResult:{providerStatus:"succeeded",modelId:MODEL,answerLength:answer.length,lineCount:lines.length}};
}
