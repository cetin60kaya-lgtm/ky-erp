...
function parseDate(message:string){
  const iso=message.match(/\b(20\d{2})[-/.](\d{1,2})[-/.](\d{1,2})\b/);if(iso)return`${iso[1]}-${iso[2].padStart(2,"0")}-${iso[3].padStart(2,"0")}`;
  const tr=message.match(/\b(\d{1,2})[./-](\d{1,2})[./-](20\d{2})\b/);if(tr)return`${tr[3]}-${tr[2].padStart(2,"0")}-${tr[1].padStart(2,"0")}`;
  return new Date().toISOString().slice(0,10);
}

function actionKind(message:string){
  const m=normalize(message);
  if(/\bACILIS\s+BAKIYE/.test(m))return"OPENING_BALANCE";
  if(/\bTAHSIL|TAHSILAT|ODEME\s+AL/.test(m))return"COLLECTION";
  if(/\bGIDER|MASRAF/.test(m)&&!/\bODEME\b/.test(m))return"MANUAL_EXPENSE";
  if(/\bODEME|ODE\b/.test(m))return"PAYMENT";
  if(/\bBORC\b/.test(m))return"DEBIT";
  if(/\bALACAK\b/.test(m))return"CREDIT";
  return"";
}

function recordScope(message:string){const m=normalize(message);return/IC KAYIT|GAYRI RESMI|GAYRIRESMI|DAHILI/.test(m)?"INTERNAL":/\bRESMI\b/.test(m)?"OFFICIAL":"INTERNAL";}
function paymentMethod(message:string){const m=normalize(message);if(/NAKIT/.test(m))return"NAKIT";if(/HAVALE|EFT|BANKA/.test(m))return"BANKA";if(/KART/.test(m))return"KART";if(/CEK/.test(m))return"CEK";return"";}
...
async function resolveCompany(c:Context<AppEnv>,slug:string,message:string){
  ...
  const aliases=(await c.env.DB.prepare(`SELECT a.company_id,a.raw_name,a.normalized_name FROM company_aliases a JOIN companies c ON c.id=a.company_id AND c.main_company_slug=a.main_company_slug WHERE a.main_company_slug=? AND COALESCE(a.is_active,1)=1`).bind(slug).all<Row>().catch(()=>({results:[]}))).results||[];
...
export async function proposeAccountingAction(c:Context<AppEnv>,slug:string,user:Row,message:string){
  const kind=actionKind(message);if(!kind)return null;
  const amount=parseAmount(message);if(!(amount>0))return{blocked:true,code:"AMOUNT_REQUIRED",message:"Muhasebe işlemi algılandı ancak tutar net değil."};
  ...
  const id=crypto.randomUUID(),scope=recordScope(message),company=resolved.company||null,method=paymentMethod(message),date=parseDate(message);
...
export async function confirmAccountingAction(c:Context<AppEnv>,slug:string,user:Row,actionId:string){
  ...
  const existing=await c.env.DB.prepare(`SELECT * FROM current_account_movements WHERE id=? AND main_company_slug=? LIMIT 1`).bind(actionId,slug).first<Row>().catch(()=>null);if(existing){const completed=await putAction(c,slug,{...action,status:"COMPLETED",completedAt:now(),result:{movementId:actionId,balanceAfter:existing.balance_after,idempotent:true}});return{ok:true,idempotent:true,action:completed,result:completed.result};}
  if(action.type==="MANUAL_EXPENSE"){
    const payload={id:actionId,date:action.date,companyId:action.companyId||null,companyName:action.companyName||"Genel gider",category:"Diğer",amount:num(action.amount),vatAmount:0,recordType:action.recordType||"GAYRI_RESMI",description:action.description,type:"EXPENSE",reportIncluded:true,addToCurrentAccount:false,paymentType:action.paymentMethod||"",note:"KY ERP AI onaylı iç kayıt",createdAt:now()};
    const ts=now();await c.env.DB.prepare(`INSERT INTO json_store(id,scope,main_company_slug,file_name,data,created_at,updated_at) VALUES(?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),"MUHASEBE_MANUAL_EXPENSE",slug,actionId,JSON.stringify(payload),ts,ts).run();const completed=await putAction(c,slug,{...action,status:"COMPLETED",completedAt:ts,result:{manualExpenseId:actionId}});return{ok:true,action:completed,result:completed.result};
  }
