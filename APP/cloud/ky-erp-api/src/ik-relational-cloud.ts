...
async function advancedPayroll(c: Context<AppEnv>) {
  ...
  const lines = employees.map((employee) => {
    ...
    const final = row ? { salaryPay: row.salary, roadPay: row.roadAllowance, overtimeAmount: row.overtimeAmount, premiumAmount: row.premiumAmount, deductionAmount: row.deductionAmount, advanceAmount: row.advanceAmount, bank: row.bankAmount, cash: row.cashAmount, total: row.totalAmount } : { salaryPay: number(employee.salary), roadPay: number(employee.roadAllowance), overtimeAmount: 0, premiumAmount: 0, deductionAmount: 0, advanceAmount: 0, bank: number(employee.bankAmount), cash: number(employee.cashAmount), total: number(employee.salary) + number(employee.roadAllowance) };
  ...
  return okData(c, { year, month, policy: { roadByActualPresence: true, defaultOvertimeBase: 225, advanceFirstFromCash: true }, lines, totals });
}
...
async function savePersonCard(c: Context<AppEnv>) {
  ...
  await c.env.DB.prepare(
    `INSERT INTO ik_person_card_settings (employee_id,main_company_id,card_no,identity_no,payroll_included,card_source,personel_kodu,exit_date,active_passive,work_type,sgk_follow,payment_type,note,phone,updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
     ON CONFLICT(employee_id) DO UPDATE SET main_company_id=excluded.main_company_id,card_no=excluded.card_no,identity_no=excluded.identity_no,payroll_included=excluded.payroll_included,card_source=excluded.card_source,personel_kodu=excluded.personel_kodu,exit_date=excluded.exit_date,active_passive=excluded.active_passive,work_type=excluded.work_type,sgk_follow=excluded.sgk_follow,payment_type=excluded.payment_type,note=excluded.note,phone=excluded.phone,updated_at=excluded.updated_at`,
  ).bind(employeeId, companyId, cardNo, text(body.identityNo), body.payrollIncluded === false ? 0 : 1, text(body.cardSource) || "TNF", text(body.personelKodu || body.code), hrDateOnly(body.exitDate) || null, text(body.activePassive || body.status) || "AKTIF", text(body.workType) || "AYLIK", body.sgkFollow === null ? 2 : body.sgkFollow === false ? 0 : 1, text(body.paymentType) || "BANKA_ELDEN", text(body.note), text(body.phone), nowIso()).run();
...
export function registerIkRelationalCloudRoutes(app: Hono<AppEnv>) {
  ...
  app.get("/api/ik/advanced/leave-center", protect(leaveCenter));
  ...
  app.post("/api/ik/advanced/leave", protect(saveLeave));
  app.post("/api/ik/advanced/leave/cancel", protect(cancelAdvancedLeave));
}
