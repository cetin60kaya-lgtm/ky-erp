...
async function fileHubContext(c:Context<AppEnv>,slug:string,message:string,moduleKey:unknown){
  try{
    const words=fileKeywords(message); const conditions:string[]=[]; const args:any[]=[slug];
...
Aktif modül: ${module}. Aktif rota: ${route||"belirtilmedi"}. Bu turda ${sourceCount} canlı tenant kaydı ve ${fileCount} File Hub dosya/ilişki satırı bağlama alındı.
Kurallar:
...
export function registerAiCloudRoutes(app:Hono<AppEnv>){
  app.get("/api/ai/status",async c=>c.json({ok:true,success:true,enabled:Boolean(c.env.AI),model:MODEL,provider:"Cloudflare Workers AI",persistentConversations:true,fileHubAware:true}));
  ...
  app.post("/api/ai/chat",async c=>{
    ...
    const [source,files]=await Promise.all([sourceContext(c,slug,pageContext.module),fileHubContext(c,slug,message,pageContext.module)]);
    ...
    const messages=[{role:"system",content:systemPrompt(pageContext,source.count,files.count)},{role:"system",content:`Canlı KY ERP iş bağlamı (JSON, sadece bu tenant): ${compactContext||"[]"}`},{role:"system",content:`File Hub dosya/ilişki bağlamı (JSON, sadece bu tenant): ${fileContext||"[]"}`},...history,{role:"user",content:message}];
