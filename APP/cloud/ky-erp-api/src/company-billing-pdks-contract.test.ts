...
test("owner-only admin workspaces stay hidden and direct routes guarded",()=>{assert.match(shell,/OWNER_ONLY_ADMIN_TABS = new Set\(\["uygulama-sahibi", "firma-ucretlendirme", "eslestirmeler", "surum-merkezi"\]\)/);assert.match(shell,/return isOwnerUser\(user\)/);assert.match(adminPage,/activeTab === "uygulama-sahibi"/);assert.match(adminPage,/owner \? <AdminOwnerSecurity \/>/)});
