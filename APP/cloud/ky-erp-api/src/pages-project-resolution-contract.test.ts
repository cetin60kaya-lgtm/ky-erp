...
test("canonical production deploy delegates to Pages auto-resolving V4", () => {
  assert.match(canonical, /KYERP_DIRECT_PRODUCTION_V4\.ps1/);
  assert.doesNotMatch(canonical, /KYERP_DIRECT_PRODUCTION_V3\.ps1/);
});
...
test("V4 keeps full auth integration in the canonical runtime preflight", () => {
  assert.match(v4, /\$source\.Replace\('npm run test:unit'/);
  assert.match(v4, /'npm test'/);
