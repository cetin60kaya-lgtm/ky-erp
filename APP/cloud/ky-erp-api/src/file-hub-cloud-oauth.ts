...
export function registerFileHubCloudOauthRoutes(app: Hono<AppEnv>) {
  ...
  app.post("/api/file-hub/cloud/authorize", async (c) => {
    ...
    let authorizeUrl = "";
    if (cfg.family === "GOOGLE") {
      const params = new URLSearchParams({ client_id: cfg.clientId, redirect_uri: cfg.redirectUri, response_type: "code", scope: cfg.scope, access_type: "offline", include_granted_scopes: "true", state });
