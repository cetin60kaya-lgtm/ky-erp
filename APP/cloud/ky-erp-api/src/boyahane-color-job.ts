...
export function registerBoyahaneColorJobRoutes(app: Hono<AppEnv>) {
  app.post("/api/boyahane/jobs/:id/colors", async (c) => {
    ...
    const registered = body.registeredColorId
      ? await storeGet(c, REGISTERED_SCOPE, text(body.registeredColorId), slug)
      : null;
    ...
    const id = crypto.randomUUID();
    const sourceType = text(registered?.sourceType || body.sourceType || body.colorSource || (body.pantone ? "PANTONE" : "VISUAL")).toUpperCase();
    ...
    await storePut(c, LOG_SCOPE, logId, {
      ...
      description: `${colorName} işe ${sourceType === "REFERENCE" ? "renk referansına göre" : sourceType === "VISUAL" ? "görsel/RGB’ye göre" : "Pantoneye göre"} eklendi.`,
