...
export function normalizeEBelgeUnit(value: unknown) {
  ...
  if (["LT", "L", "LITRE", "LITER"].includes(unit)) return "LT";
  if (["M", "METRE", "MTR"].includes(unit)) return "M";
  return unit || "ADET";
}

function tokenSimilarity(left: unknown, right: unknown) {
  const a = new Set(normalizeEBelgeText(left).split(" ").filter(Boolean));
  const b = new Set(normalizeEBelgeText(right).split(" ").filter(Boolean));
...
function productName(line: EBelgeLine) {
  return normalizeEBelgeText(line.description || line.productCode || line.supplierProductCode || line.productId || "");
}
...
export function compareEBelgeProducts(invoiceLine: EBelgeLine, dispatchLine: EBelgeLine) {
  ...
  const invoiceSupplierCode = normalizeEBelgeText(invoiceLine.supplierProductCode);
  const dispatchSupplierCode = normalizeEBelgeText(dispatchLine.supplierProductCode);
  if (invoiceSupplierCode && dispatchSupplierCode && invoiceSupplierCode === dispatchSupplierCode) {
    return { score: 97, source: "SUPPLIER_PRODUCT_CODE" };
  }
  const invoiceName = productName(invoiceLine);
  const dispatchName = productName(dispatchLine);
...
function sameParty(invoice: EBelgeDocument, dispatch: EBelgeDocument) {
  const invoiceCompany = String(invoice.partyCompanyId || "").trim();
  const dispatchCompany = String(dispatch.partyCompanyId || "").trim();
  if (invoiceCompany && dispatchCompany) return invoiceCompany === dispatchCompany;
  const invoiceTax = String(invoice.partyTaxNo || "").replace(/\D/g, "");
  const dispatchTax = String(dispatch.partyTaxNo || "").replace(/\D/g, "");
  return Boolean(invoiceTax && dispatchTax && invoiceTax === dispatchTax);
}

function sameDirection(invoice: EBelgeDocument, dispatch: EBelgeDocument) {
  const left = normalizeEBelgeText(invoice.direction);
  const right = normalizeEBelgeText(dispatch.direction);
...
export function reconcileEBelgeInvoice(
  ...
  const refs = [...new Set(explicitReferences.map(normalizeEBelgeText).filter(Boolean))];
  const invoiceLines = Array.isArray(invoice.lines) ? invoice.lines : [];
  const ranked = allDispatches
    .filter((row) => row?.id && row.id !== invoice.id && sameDirection(invoice, row))
    .map((row) => {
      const normalizedNo = normalizeEBelgeText(row.documentNo);
    ...
    .filter((candidate) => {
      if (!candidate.party) return false;
      if (refs.length) return candidate.explicit || candidate.days <= 90;
      return candidate.days <= 60;
    })
    .sort((a, b) => b.score - a.score || a.days - b.days);

  const candidates = refs.length
    ? [...ranked.filter((row) => row.explicit), ...ranked.filter((row) => !row.explicit)]
    : ranked;
  const availability = new Map<string, number>();
  for (const candidate of candidates) {
    for (const line of candidate.row.lines || []) {
      availability.set(`${candidate.row.id}:${line.id || "line"}`, Math.max(0, numberValue(line.quantity)));
    }
  }

  const lineResults: EBelgeLineResult[] = invoiceLines.map((invoiceLine, index) => {
    const targetQty = Math.max(0, numberValue(invoiceLine.quantity));
    const unit = normalizeEBelgeUnit(invoiceLine.unitCode);
    let remaining = targetQty;
    const allocations: EBelgeAllocation[] = [];
    const lineCandidates = candidates
      .flatMap((candidate) => (candidate.row.lines || []).map((dispatchLine) => ({
        candidate,
        dispatchLine,
        product: compareEBelgeProducts(invoiceLine, dispatchLine),
        unitCompatible: normalizeEBelgeUnit(dispatchLine.unitCode) === unit,
      })))
    ...
    else if (difference < -0.0005) status = "PARTIAL";
    return {
      invoiceLineId: String(invoiceLine.id || `invoice-line-${index + 1}`),
      invoiceProduct: String(invoiceLine.description || invoiceLine.productCode || invoiceLine.supplierProductCode || "Kalem"),
  ...
  const linkedDispatches = candidates
    ...
    .map((candidate) => {
      ...
      return {
        id: candidate.row.id,
        documentNo: String(candidate.row.documentNo || ""),
