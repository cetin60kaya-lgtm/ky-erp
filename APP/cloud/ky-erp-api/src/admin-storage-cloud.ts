...
export function registerAdminStorageRoutes(app:any){
  ...
  app.get("/api/admin/file-storage/status",async(c:any)=>{
    const owner=await ownerCurrent(c); if(!owner) return c.json(errorBody("OWNER_ONLY","Dosya ve depolama yönetimi yalnız uygulama sahibine açıktır."),403);
    return c.json({ok:true,data:await legacyStatus(c)});
  });
  app.get("/api/admin/file-storage/settings",async(c:any)=>{
    const owner=await ownerCurrent(c); if(!owner) return c.json(errorBody("OWNER_ONLY","Dosya ve depolama yönetimi yalnız uygulama sahibine açıktır."),403);
    return c.json({ok:true,data:{storageRoot:`FILE_HUB://${slugOf(c)}`,storageMode:"FILE_HUB",note:"Ana dosya evi firma bazında Google Drive, OneDrive veya seçilen provider'dır. R2 yalnız preview/cache katmanıdır."}});
  });
  app.get("/api/admin/file-storage/rules",async(c:any)=>{
    const owner=await ownerCurrent(c); if(!owner) return c.json(errorBody("OWNER_ONLY","Dosya kuralları yalnız uygulama sahibine açıktır."),403);
    const slug=slugOf(c); const r=await c.env.DB.prepare(`SELECT b.id,b.module_code module,b.purpose_code documentType,b.root_path targetPathTemplate,b.read_enabled,b.write_enabled,b.sync_enabled,c.provider_type,c.name providerName FROM file_hub_bindings b JOIN file_hub_connections c ON c.id=b.storage_connection_id WHERE b.main_company_slug=? ORDER BY b.module_code,b.purpose_code`).bind(slug).all<Row>();
    return c.json({ok:true,data:r.results||[]});
  });
  app.get("/api/admin/file-storage/files",async(c:any)=>{
    const owner=await ownerCurrent(c); if(!owner) return c.json(errorBody("OWNER_ONLY","Dosya listesi yalnız uygulama sahibine açıktır."),403);
    const slug=slugOf(c),take=Math.min(500,Math.max(1,Number(c.req.query("take")||100))); const r=await c.env.DB.prepare(`SELECT a.id,a.file_name fileName,a.size_bytes fileSize,a.status,a.updated_at uploadedAt,l.relative_path fileKey,c.provider_type sourceType FROM file_hub_assets a LEFT JOIN file_hub_locations l ON l.file_asset_id=a.id AND l.location_role='PRIMARY' LEFT JOIN file_hub_connections c ON c.id=l.storage_connection_id WHERE a.main_company_slug=? ORDER BY a.updated_at DESC LIMIT ?`).bind(slug,take).all<Row>();
    return c.json({ok:true,data:r.results||[]});
  });
}
