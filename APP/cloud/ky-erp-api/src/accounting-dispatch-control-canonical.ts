...
async function build(c:Context<AppEnv>){
  const slug=slugOf(c);if(!slug)throw Object.assign(new Error("Ana firma seçimi zorunludur."),{code:"MAIN_COMPANY_REQUIRED"});
...
export function registerCanonicalDispatchControlRoutes(app:Hono<AppEnv>){
  app.get("/api/muhasebe/customer-dispatches/summary",async c=>{try{const data=await build(c);return c.json({ok:true,success:true,data:data.summary,readModel:data.readModel})}catch(error:any){return c.json({ok:false,error:{code:text(error?.code)||"DISPATCH_CONTROL_FAILED",message:text(error?.message)||"İrsaliye/fatura kontrolü hazırlanamadı."}},text(error?.code)==="MAIN_COMPANY_REQUIRED"?400:500)}});
  app.get("/api/muhasebe/customer-dispatches",async c=>{try{const data=await build(c);return c.json({ok:true,success:true,data})}catch(error:any){return c.json({ok:false,error:{code:text(error?.code)||"DISPATCH_CONTROL_FAILED",message:text(error?.message)||"İrsaliye/fatura kontrolü hazırlanamadı."}},text(error?.code)==="MAIN_COMPANY_REQUIRED"?400:500)}});
}
