...
export function registerMailCommunicationRoutes(app:any){
  ...
  app.get("/api/mail/overview",async(c:any)=>{
    ...
    const messages=await c.env.DB.prepare("SELECT COUNT(*) n,SUM(CASE WHEN m.direction='INCOMING' AND m.is_read=0 AND (UPPER(COALESCE(f.folder_type,''))='INBOX' OR UPPER(COALESCE(f.provider_folder_id,''))='INBOX') THEN 1 ELSE 0 END) unread_n FROM mail_messages m JOIN mail_accounts a ON a.id=m.account_id AND a.main_company_slug=m.main_company_slug LEFT JOIN mail_folders f ON f.id=m.folder_id AND f.main_company_slug=m.main_company_slug WHERE "+accountWhere).bind(...args).first<AnyRow>();
