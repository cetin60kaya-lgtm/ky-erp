-- KY ERP PDKS operasyon tablolari icin D1 readiness.
-- CREATE IF NOT EXISTS kullanir; mevcut veriyi degistirmez/silmez.
...
CREATE INDEX IF NOT EXISTS idx_ik_leave_plans_pdks_period ON ik_leave_plans(main_company_id,start_date,end_date,status);
CREATE INDEX IF NOT EXISTS idx_ik_leave_plans_pdks_employee ON ik_leave_plans(employee_id,start_date,end_date);
...
CREATE TABLE IF NOT EXISTS hr_monthly_adjustments_v2 (
  ...
  payment_method TEXT NOT NULL DEFAULT 'Elden',
  payroll_effect TEXT NOT NULL DEFAULT '',
  note TEXT NOT NULL DEFAULT '',
  status TEXT NOT NULL DEFAULT 'APPROVED'
);
CREATE INDEX IF NOT EXISTS idx_hr_monthly_adjustments_pdks_employee_date ON hr_monthly_adjustments_v2(employee_id,date);
...
CREATE INDEX IF NOT EXISTS idx_ik_audit_logs_pdks ON ik_audit_logs(main_company_id,employee_id,period,created_at);
