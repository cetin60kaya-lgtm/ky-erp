...
namespace KyPdksFpClockProbe
{
    ...
    internal sealed class ProbeForm : Form
    {
        ...
        private readonly Button _run = new Button { Text = "FP_CLOCK İLE DOĞRUDAN BAĞLAN", AutoSize = true, Height = 34 };
        ...
        public ProbeForm()
        {
            Text = "KY PDKS · FP_CLOCK Doğrudan Cihaz Okuma Testi v4";
            Width = 1120;
            Height = 740;
            ...
            var note = new Label
            {
                ...
                Text = "SALT OKUMA / TANILAMA v4: Windows COM kaydı veya registration-free manifest kullanmaz. FP_CLOCK.ocx'i gerçek bulunduğu klasörden LOAD_WITH_ALTERED_SEARCH_PATH ile yükler ve DllGetClassObject üzerinden doğrudan sınıf fabrikasını açar. Cihaza saat/zil/kapı/silme/reset komutu göndermez."
            };
        ...
        private void RunProbe()
        {
            ...
            try
            {
                ...
                Log("KY PDKS - FP_CLOCK DOĞRUDAN CİHAZ OKUMA TESTİ v4.0");
                ...
                string searchPath = string.Join(";", new[] { supportDir, appDir, parentDir }.Where(Directory.Exists).Distinct(StringComparer.OrdinalIgnoreCase));
                Environment.SetEnvironmentVariable("PATH", searchPath + ";" + oldPath);
                SetDllDirectory(supportDir);
                Log("DLL arama yolu öne alındı: " + searchPath);

                if (File.Exists(dep))
                {
                    hTmp = LoadLibraryEx(dep, IntPtr.Zero, LOAD_WITH_ALTERED_SEARCH_PATH);
                    if (hTmp == IntPtr.Zero)
                        Log("TMPCCOMM.dll LoadLibraryEx HATA · Win32=" + Marshal.GetLastWin32Error());
                    else
                        Log("TMPCCOMM.dll YÜKLENDİ.");
                }

                hOcx = LoadLibraryEx(ocxPath, IntPtr.Zero, LOAD_WITH_ALTERED_SEARCH_PATH);
                if (hOcx == IntPtr.Zero)
                    throw new InvalidOperationException("FP_CLOCK.ocx LoadLibraryEx başarısız. Win32=" + Marshal.GetLastWin32Error() + " (126=bağımlı modül bulunamadı)." );
                ...
                Log("FP_CLOCK COM NESNESİ DOĞRUDAN OLUŞTURULDU.");
                ...
                {
                    Log("SONUÇ: OCX doğrudan yüklendi; cihaz çağrısı yapıldı fakat GetDeviceTime ile oturum henüz doğrulanamadı.");
                    _state.Text = "OCX OK / CİHAZ DOĞRULANMADI";
                }
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                ...
                try { if (hOcx != IntPtr.Zero) FreeLibrary(hOcx); } catch { }
                try { if (hTmp != IntPtr.Zero) FreeLibrary(hTmp); } catch { }
                try { SetDllDirectory(null); Environment.SetEnvironmentVariable("PATH", oldPath); } catch { }

                string report = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "KY-PDKS-FPClock-Cihaz-Raporu-v4.txt");
        ...
        private string FindFpClock()
        {
            ...
            foreach (Process p in Process.GetProcesses())
            {
                try
                {
                    ...
                    if (title.IndexOf("Hedef", StringComparison.OrdinalIgnoreCase) < 0 && title.IndexOf("Terminal", StringComparison.OrdinalIgnoreCase) < 0 && name.IndexOf("Hedef", StringComparison.OrdinalIgnoreCase) < 0 && name.IndexOf("Terminal", StringComparison.OrdinalIgnoreCase) < 0) continue;
            ...
            string[] known = {
                @"C:\Hedef500\Terminal Bilgi Aktar\support\FP_CLOCK.ocx", @"D:\Hedef500\Terminal Bilgi Aktar\support\FP_CLOCK.ocx", @"E:\Hedef500\Terminal Bilgi Aktar\support\FP_CLOCK.ocx",
                @"C:\HEDEF\Terminal Bilgi Aktar\support\FP_CLOCK.ocx", @"D:\HEDEF\Terminal Bilgi Aktar\support\FP_CLOCK.ocx", @"E:\HEDEF\Terminal Bilgi Aktar\support\FP_CLOCK.ocx",
        ...
        {
            var q = new Queue<Tuple<string, int>>(); q.Enqueue(Tuple.Create(root, 0)); int scanned = 0; const int maxDirs = 30000;
            while (q.Count > 0 && scanned < maxDirs)
            {
                var item = q.Dequeue(); string dir = item.Item1; int depth = item.Item2; scanned++;
                try
                {
                    string direct = Path.Combine(dir, "FP_CLOCK.ocx");
                    if (File.Exists(direct)) { Log("Disk taramasında BULUNDU: " + direct); return direct; }
                    if (depth >= 6) continue;
                    foreach (string sub in Directory.EnumerateDirectories(dir))
                    {
                        ...
                        bool priority = n.IndexOf("hedef", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("terminal", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("pdks", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("support", StringComparison.OrdinalIgnoreCase) >= 0 || n.IndexOf("personel", StringComparison.OrdinalIgnoreCase) >= 0;
                        if (priority || depth < 3) q.Enqueue(Tuple.Create(sub, depth + 1));
                    }
                }
                catch { }
            }
            Log("  Taranan klasör: " + scanned); return null;
        }

        private List<ComCandidate> InspectTypeLibrary(string path)
        {
            var result = new List<ComCandidate>(); ComTypes.ITypeLib lib; int hr = LoadTypeLibEx(path, REGKIND.REGKIND_NONE, out lib); if (hr != 0 || lib == null) Marshal.ThrowExceptionForHR(hr);
            try
            {
                int count = lib.GetTypeInfoCount(); Log("TypeLib yüklendi · TypeInfo=" + count);
                for (int i = 0; i < count; i++)
                {
                    lib.GetTypeInfoType(i, out ComTypes.TYPEKIND kind); if (kind != ComTypes.TYPEKIND.TKIND_COCLASS) continue;
                    lib.GetTypeInfo(i, out ComTypes.ITypeInfo ti); IntPtr pAttr = IntPtr.Zero;
                    try
                    {
                        ti.GetTypeAttr(out pAttr); var attr = (ComTypes.TYPEATTR)Marshal.PtrToStructure(pAttr, typeof(ComTypes.TYPEATTR)); ti.GetDocumentation(-1, out string name, out _, out _, out _); var methods = new List<MethodDesc>();
                        for (int impl = 0; impl < attr.cImplTypes; impl++)
                        {
                            try { ti.GetRefTypeOfImplType(impl, out int href); ti.GetRefTypeInfo(href, out ComTypes.ITypeInfo iface); try { CollectMethods(iface, methods); } finally { if (Marshal.IsComObject(iface)) Marshal.ReleaseComObject(iface); } } catch { }
                        }
                        result.Add(new ComCandidate { Name = name ?? "COM", Guid = attr.guid, Methods = methods });
                    }
                    finally { if (pAttr != IntPtr.Zero) ti.ReleaseTypeAttr(pAttr); if (Marshal.IsComObject(ti)) Marshal.ReleaseComObject(ti); }
                }
            }
        ...
        private static void CollectMethods(ComTypes.ITypeInfo ti, List<MethodDesc> methods)
        {
            ...
            try
            {
                ti.GetTypeAttr(out pAttr); var attr = (ComTypes.TYPEATTR)Marshal.PtrToStructure(pAttr, typeof(ComTypes.TYPEATTR));
                for (int i = 0; i < attr.cFuncs; i++)
                {
                    IntPtr pFunc = IntPtr.Zero;
                    try { ti.GetFuncDesc(i, out pFunc); var fd = (ComTypes.FUNCDESC)Marshal.PtrToStructure(pFunc, typeof(ComTypes.FUNCDESC)); var names = new string[Math.Max(1, fd.cParams + 1)]; ti.GetNames(fd.memid, names, names.Length, out int got); if (got > 0 && !string.IsNullOrWhiteSpace(names[0])) methods.Add(new MethodDesc { Name = names[0], ParamCount = fd.cParams }); }
                    catch { }
        ...
        private object CallSetIp(Type t, object com, int pc, string ip, int port, int dn, int key)
        {
            ...
            Exception last = null;
            foreach (object[] a in new[] { new object[] { ip, port, key }, new object[] { ip, port }, new object[] { ip, port, dn, key } }) { try { return Invoke(t, com, "SetIPAddress", a); } catch (Exception ex) { last = ex; } }
        ...
        private object CallOpen(Type t, object com, int pc, int dn)
        {
            ...
            Exception last = null;
            foreach (object[] a in new[] { new object[] { dn }, new object[0] }) { try { return Invoke(t, com, "OpenCommPort", a); } catch (Exception ex) { last = ex; } }
        ...
        {
            var m = c.Methods.FirstOrDefault(x => Eq(x.Name, "GetDeviceTime")); if (m == null) { Log("GetDeviceTime: metot yok"); return false; }
            try
            {
                object[] a = { dn, 0, 0, 0, 0, 0, 0 }; var pm = new ParameterModifier(a.Length); for (int i = 1; i < a.Length; i++) pm[i] = true;
        ...
        private void TryReadStatus(Type t, object com, ComCandidate c, int dn)
        {
            ...
            {
                try { object[] a = { dn, code, 0 }; var pm = new ParameterModifier(3); pm[2] = true; object ret = t.InvokeMember("GetDeviceStatus", BindingFlags.InvokeMethod, null, com, a, new[] { pm }, CultureInfo.InvariantCulture, null); Log($"  Kod {code}: {a[2]} · sonuç={Format(ret)}"); }
        ...
        private void TryReadInfo(Type t, object com, ComCandidate c, int dn)
        {
            ...
            {
                try { object[] a = { dn, code, 0 }; var pm = new ParameterModifier(3); pm[2] = true; object ret = t.InvokeMember("GetDeviceInfo", BindingFlags.InvokeMethod, null, com, a, new[] { pm }, CultureInfo.InvariantCulture, null); Log($"  Kod {code}: {a[2]} · sonuç={Format(ret)}"); }
                catch (Exception ex) { Log($"  Kod {code}: HATA · {Unwrap(ex).Message}"); }
            }
        }
        ...
        private static bool IsInteresting(string s) { string x = (s ?? "").ToLowerInvariant(); return x.Contains("comm") || x.Contains("device") || x.Contains("ipaddress") || x.Contains("log") || x.Contains("bell") || x.Contains("door") || x.Contains("enroll") || x.Contains("user"); }
