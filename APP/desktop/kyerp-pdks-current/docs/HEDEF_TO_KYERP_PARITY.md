...
| Puantaj hesaplama | Günlük/Aylık Puantaj + Bilgi sekmesi | YARIM | Puantaj | `GIRCIK`, `OZELIZIN`, `PERPLANTAT`, `GRUP`, `PUANTAJ` | Transactional günlük üretim ve toplamlar var; `YUVARLA`/ceza katsayıları ile kapanış eksik |
| Bordro | Genel Maaş Bordrosu + Ödemeler | YARIM | Bordro | `PUANTAJ`, `AVANS`, `ODEME`, `KIMLIK` | Gün/mesai/net toplu hesap, print ve PDF/XLSX var; dönem kapanışı eksik |
...
| Terminal kayıt aktarımı | Legacy Terminal Veri Transferi dialogu | YARIM | Terminal | Profile-driven File/TNF adapter + toleranslı transaction `GIRCIK` import tamam; fiziksel cihaz adapterı bekliyor | 5 dakika duplicate penceresi + rollback smoke |
...
| Kişisel ve operasyon raporları | Raporlar menüsü | YARIM | Raporlar | `KIMLIK`, `AVANS`, `OZELIZIN`, `PUANTAJ` ve tanımlar | 6 kişisel + 5 doğrudan operasyon raporu; FR3 birebir render kapsamı eksik |
