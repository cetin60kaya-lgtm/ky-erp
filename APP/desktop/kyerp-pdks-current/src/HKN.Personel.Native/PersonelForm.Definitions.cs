...
public partial class PersonelForm
{
    ...
    [
        new("Grup","GRUP","GRUP"),new("Bölüm","BOLUM","BOLUM"),new("Durum","DURUM","DURUM"),
        new("Servis","SERVIS","SERVIS"),new("Görev","GOREV","GOREV"),new("Firma","FIRMA","SIRKET")
    ];
    ...
    void ShowOrganizationDefinitions()
    {
        ...
        delete.Click+=(_,_)=>Run(()=>{var value=Selected();var row=SelectedRow(grid);var code=row.Cells["KOD"].Value;var used=Convert.ToInt32(db.Scalar($"select count(*) from KIMLIK where {value.EmployeeColumn}=@K",new FbParameter("@K",code)));if(used>0)throw new InvalidOperationException($"Bu tanım {used} personelde kullanılıyor; silinemez.");if(MessageBox.Show("Seçili tanım silinsin mi?","Organizasyon Tanımları",MessageBoxButtons.YesNo,MessageBoxIcon.Warning)==DialogResult.Yes)Exec($"delete from {value.Table} where KOD=@K",new FbParameter("@K",code));});
    ...
    sealed record DefinitionType(string Label,string Table,string EmployeeColumn);
