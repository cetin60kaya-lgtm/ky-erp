...
export async function uploadFaturaKesimPdf(activeMainCompany, file) {
  const form = new FormData();
  form.append("file", file);
  const company = withCompany(activeMainCompany);
  if (company?.mainCompanySlug)
    form.append("mainCompanySlug", company?.mainCompanySlug);
  if (company?.mainCompanyId)
    form.append("mainCompanyId", company?.mainCompanyId);
  return unwrap(await apiUpload("/muhasebe/fatura-kesim/pdf-oku", form));
}

export async function getFaturaKesimHavuz(activeMainCompany, filters = {}) {
  return unwrap(
    await apiGet(
      "/muhasebe/fatura-kesim/havuz",
      withCompany(activeMainCompany, filters),
    ),
  );
}

export async function getFaturaKesimDetail(activeMainCompany, id) {
  return unwrap(
    await apiGet(
      `/muhasebe/fatura-kesim/${encodeURIComponent(id)}`,
      withCompany(activeMainCompany),
    ),
  );
}

export async function linkFaturaKesimModel(
  activeMainCompany,
  id,
  payload = {},
) {
  return unwrap(
    await apiPost(
      `/muhasebe/fatura-kesim/${encodeURIComponent(id)}/link-model`,
      {
        ...withCompany(activeMainCompany),
        ...payload,
      },
...
export async function saveFaturaKesimInvoice(
  activeMainCompany,
  id,
  payload = {},
) {
  return unwrap(
    await apiPost(
      `/muhasebe/fatura-kesim/${encodeURIComponent(id)}/fatura-kaydi`,
      {
        ...withCompany(activeMainCompany),
        ...payload,
      },
...
export async function uploadFaturaKesimIsnetFiles(
  activeMainCompany,
  id,
  files = [],
  payload = {},
  ...
  if (company?.mainCompanyId)
    form.append("mainCompanyId", company?.mainCompanyId);
  if (company?.lineId) form.append("lineId", company?.lineId);
  if (company?.invoiceOfficerNote)
    form.append("invoiceOfficerNote", company?.invoiceOfficerNote);
  return unwrap(
    await apiUpload(
      `/muhasebe/fatura-kesim/${encodeURIComponent(id)}/isnet-dosyalari`,
      form,
    ),
  );
}
