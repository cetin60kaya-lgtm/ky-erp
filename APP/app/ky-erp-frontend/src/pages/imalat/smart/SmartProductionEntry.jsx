...
export default function SmartProductionEntry({ activeMainCompany, onConfigureMachines }) {
  const storageKey = `kyerp-smart-production-${activeMainCompany?.slug || activeMainCompany?.id || "default"}`;
  ...
  useEffect(() => {
    ...
    getProductionParserDictionaries(activeMainCompany)
      .then((data) => {
        if (cancelled) return;
        const dictionaries = data || { models: [], machines: [], operators: [] };
        setDict(dictionaries);
        const firstMachine = dictionaries.machines?.[0];
        if (firstMachine) {
          setDefaults((current) => ({
            ...current,
            machineId: machineId(firstMachine),
            machineName: machineName(firstMachine),
            operatorName: operatorForMachine(firstMachine, current.shift),
          }));
        }
      })
      .catch((error) => setMessage(error?.message || "Üretim sözlükleri okunamadı."));
    loadRecent();
    return () => {
      cancelled = true;
    };
  }, [activeMainCompany, loadRecent]);
  ...
  function parse() {
    ...
    setMessage(
      issueCount
        ...
        : `${productionRows.length} satır çözümlendi ve kayda hazır.`,
    );
  }
    ...
    <div className="smart-entry">
      <section className="iw-card smart-capture-card">
        <div className="iw-card-head">
          <div>
            <h2><ScanText size={18} /> Akıllı Seri Fiş Girişi</h2>
            <small>Satır sırası serbesttir. “gündüz 1” yazıldığında Makine 1'in kayıtlı gündüz makinacısı otomatik kullanılır.</small>
          </div>
        ...
        <div className="iw-card-body">
          ...
          {!dict.machines.length ? (
            <div className="smart-machine-warning">
              <div><CircleAlert size={17} /><span><strong>Aktif makine tanımı yok.</strong> Seri kayıt için önce makine no, adı, gündüz ve gece makinacısını tanımlayın.</span></div>
              <button className="iw-btn" type="button" onClick={onConfigureMachines}>Makine Ayarlarını Aç</button>
            </div>
          ) : null}
          <textarea
          ...
          <div className="seri-actions smart-primary-actions">
            ...
            <button className="iw-btn" type="button" onClick={loadRecent}><History size={16} /> Kayıtları Yenile</button>
          </div>
      ...
      {rows.length ? (
        <section className="iw-card smart-review-card">
          <div className="iw-card-head smart-review-head">
            <div>
              <h2><ClipboardCheck size={18} /> Seri Hata Denetimi</h2>
              <small>Her satır kayıttan önce zorunlu alan, tarih/adet ve yinelenen kayıt açısından kontrol edilir.</small>
            </div>
            <div className="seri-actions">
              <button className="iw-btn" type="button" onClick={runSerialValidation}><ClipboardCheck size={16} /> Yeniden Denetle</button>
              <button className={`iw-btn ${showIssuesOnly ? "active-filter" : ""}`} type="button" onClick={() => setShowIssuesOnly((current) => !current)}><CircleAlert size={16} /> {showIssuesOnly ? "Tüm Satırlar" : "Sorunlular"}</button>
              <button className="iw-btn primary" type="button" onClick={saveReadyRows}><Save size={16} /> Hazırları Kaydet</button>
            </div>
          </div>
          <div className="iw-card-body smart-summary-body"><ProductionEntrySummary rows={rows} /></div>
          <ParsedProductionTable rows={visibleRows} setRows={setRows} dictionaries={dict} />
        </section>
      ) : null}

      <section className="iw-card">
        <div className="iw-card-head"><h2><History size={18} /> Son 20 Üretim Kaydı</h2></div>
        <div className="iw-table-wrap compact">
          <table>
            <thead><tr><th>Tarih</th><th>Model</th><th>Görsel</th><th>Bölge</th><th>Adet</th><th>Makine</th><th>Vardiya</th><th>Makinacı</th></tr></thead>
            <tbody>
              {recent.length ? recent.map((entry) => (
                <tr key={entry.id}>
                  <td>{entry.tarih}</td><td>{entry.model}</td>
                  <td>{entry.modelImageUrl ? <img className="smart-thumb" src={entry.modelImageUrl} alt={entry.model || "Model"} /> : "-"}</td>
                  <td>{entry.baskiBolgesi || "-"}</td><td>{Number(entry.adet || 0).toLocaleString("tr-TR")}</td>
                  <td>{[entry.makineNo, entry.makineAdi].filter(Boolean).join(" - ") || "-"}</td><td>{entry.vardiya || "-"}</td><td>{entry.makinaci || "-"}</td>
                </tr>
              )) : <tr><td colSpan={8}>Henüz üretim kaydı bulunamadı.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
