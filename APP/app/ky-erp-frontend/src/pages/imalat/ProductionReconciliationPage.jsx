...
function tone(status) {
  ...
  if (["EKSİK", "EKSİK BÖLGE", "İRSALİYE YOK"].includes(status)) return "yellow";
...
export default function ProductionReconciliationPage({ activeMainCompany }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [result, companyRows] = await Promise.all([
        getProductionReconciliation(activeMainCompany, filters),
        companies.length ? Promise.resolve(companies) : getDesignCompanies(activeMainCompany),
      ]);
      setData({
  ...
  }, [activeMainCompany, companies, filters]);
  ...
  async function createModel() {
    ...
    try {
      const result = await quickCreateCanonicalModel(activeMainCompany, {
        ...
        firmaAdi: company?.name || createForm.companyName,
        companyName: company?.name || createForm.companyName,
        sourceModule: "IMALAT",
      });
      setCreateOpen(false);
