...
function flattenRows(rows = []) {
  return rows.flatMap((row) => {
    const operations =
      Array.isArray(row.operationRows) && row.operationRows.length
        ? row.operationRows
        : [
            {
              region: "Ön",
              producedQty: row.completedGrossQty || 0,
              missingQty: row.remainingQty || 0,
            },
          ];
    ...
    return operations.map((operation) => ({
      ...
      remainingQty: Math.max(
        0,
        number(row.expectedQty) - number(operation.producedQty),
      ),
      overQty: Math.max(
        0,
...
export default function ProductionWorkPoolPage({ activeMainCompany }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const nextMachines = Array.isArray(dictionaries?.machines)
      ...
      setSelectedId((current) =>
        nextJobs.some((row) => row.id === current)
          ? current
          : nextJobs[0]?.id || "",
      );
      ...
      if (firstMachine) {
        setEntry((current) => ({
          ...
          makinaci:
            current.makinaci || operatorFor(firstMachine, current.vardiya),
        }));
      }
      setMessage("");
    } catch (error) {
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter((row) =>
      `${row.modelName} ${row.companyName} ${row.dispatchNo} ${row.printArea}`
  ...
  const selectedMachine = machines.find(
    (row) => machineId(row) === entry.makineNo,
  );
  const totalDefect =
    number(entry.baskiHatasiAdet) + number(entry.kumasHatasiAdet);
  ...
  async function saveEntry() {
    ...
    if (
      ...
      setMessage(
        "Tarih, makine, makinacı ve brüt üretim adedi zorunludur.",
      );
      return;
    }
    ...
    try {
      ...
      setMessage(
        `${selected.modelName} · ${selected.printArea}: ${qty(entry.adet)} brüt, ${qty(netEntry)} net sağlam kaydedildi.`,
      );
      setEntry((current) => ({
        ...current,
        adet: "",
  ...
  async function createManualJob() {
    if (
      !manual.modelName.trim() ||
      !manual.firma.trim() ||
      number(manual.expectedQty) <= 0
    ) {
      setMessage(
        "İrsaliyesiz iş için model adı, firma ve beklenen adet zorunludur.",
      );
      return;
    }
    ...
    try {
      ...
      setMessage(
        "İrsaliyesiz iş Desen merkezinde açıldı ve üretim havuzuna eklendi.",
      );
      await load();
    } catch (error) {
    ...
    <div className="production-work-pool">
      <section className="iw-card production-work-toolbar">
        <div className="production-search">
          ...
          <input
            ...
            placeholder="Model, firma, irsaliye veya baskı bölgesi ara"
          />
        </div>
        <span className="production-page-intro-badge">
          <PackageCheck size={15} /> {filtered.length} operasyon
        </span>
        <button
          className="iw-btn"
          type="button"
        ...
        >
          <Plus size={15} /> İrsaliyesiz İş Aç
        </button>
      </section>

      {message ? (
        <div className="iw-notice production-notice">
      ...
      <div className="production-work-layout">
        <section className="iw-card production-job-list-card">
          <div className="iw-card-head">
            <div>
              <h2>
                <Factory size={18} /> Açık Operasyonlar
              </h2>
              <small>
                Her baskı bölgesi ayrı kayıt alır; model toplamı denge
                ekranında ortak hesaplanır.
              </small>
            </div>
          </div>
            ...
            {filtered.length ? (
              filtered.map((job) => (
                <button
                  type="button"
                  key={job.id}
                  className={`production-job-item ${selected?.id === job.id ? "active" : ""}`}
                  onClick={() => setSelectedId(job.id)}
                >
                  <span>
                    <strong>{job.modelName}</strong>
                    <small>
                      {job.companyName || "Firma Seçilmedi"} ·{" "}
                      {job.dispatchNo || "İrsaliye yok"}
                    </small>
                  </span>
                  <span>
                    <b>{job.printArea}</b>
                    <small>
                      {qty(job.producedQty)} / {qty(job.expectedQty)}
                    </small>
                  </span>
              ...
              <div className="iw-empty">
                {loading
                  ? "İş havuzu hazırlanıyor..."
                  : "Açık üretim operasyonu yok."}
              </div>
            )}
          </div>
        </section>
          ...
          {selected ? (
            <>
              <div className="iw-card-head">
                <div>
                  <h2>
                    {selected.modelName} · {selected.printArea}
                  </h2>
                  <small>
                    {selected.companyName || "Firma Seçilmedi"} ·{" "}
                    {selected.dispatchNo || "İrsaliye yok"}
                  </small>
                </div>
                <span
                  className={`production-status ${statusTone(selected.status)}`}
                >
                  {selected.status}
                </span>
              </div>

              <div className="production-selected-metrics">
                <span>
                  Gelen <b>{qty(selected.expectedQty)}</b>
                </span>
                <span>
                  Üretilen <b>{qty(selected.producedQty)}</b>
                </span>
                <span>
                  Kalan <b>{qty(selected.remainingQty)}</b>
                </span>
                <span>
                  Fazla <b>{qty(selected.overQty)}</b>
                </span>
              </div>
              ...
              <div className="iw-card-body production-entry-grid">
                <label>
                  ...
                  <input
                    ...
                    value={entry.tarih}
                    onChange={(event) =>
                      setEntry({ ...entry, tarih: event.target.value })
                    }
                  />
                </label>
                <label>
                  ...
                  >
                    ...
                    {machines.map((machine) => (
                      <option
                        ...
                        value={machineId(machine)}
                      >
                        {machine.machineNo || machine.makineNo || machine.no} -{" "}
                        {machineName(machine)}
                      </option>
                    ))}
                  </select>
                ...
                <label>
                  <span>Makinacı *</span>
                  <input
                    value={entry.makinaci}
                    onChange={(event) =>
                      setEntry({ ...entry, makinaci: event.target.value })
                    }
                  />
                </label>
                <label>
                  <span>Brüt Üretim *</span>
                  <input
                    type="number"
                    min="0"
                    value={entry.adet}
                    onChange={(event) =>
                      setEntry({ ...entry, adet: event.target.value })
                    }
                  />
                </label>
                <label className="defect">
                  <span>Baskı Sakatı</span>
                  <input
                    type="number"
                    min="0"
                    value={entry.baskiHatasiAdet}
                    onChange={(event) =>
                      setEntry({
                        ...entry,
                        baskiHatasiAdet: event.target.value,
                      })
                    }
                  />
                </label>
                <label className="defect">
                  <span>Kumaş Sakatı</span>
                  <input
                    type="number"
                    min="0"
                    value={entry.kumasHatasiAdet}
                    onChange={(event) =>
                      setEntry({
                        ...entry,
                        kumasHatasiAdet: event.target.value,
                      })
                    }
                  />
                </label>
                <label>
                  <span>Net Sağlam</span>
                  <input value={qty(netEntry)} disabled />
                </label>
                <label className="span-2">
                  ...
                  <textarea
                    value={entry.not}
                    onChange={(event) =>
                      setEntry({ ...entry, not: event.target.value })
                    }
                  />
                </label>
              </div>

              <footer className="production-entry-footer">
                <span>
                  <TriangleAlert size={15} /> Sakatlar brüt adetten düşülür;
                  irsaliye farkı denge ekranına yansır.
                </span>
                <button
                  className="iw-btn primary"
                  type="button"
                ...
                >
                  <Save size={16} />
                  {saving ? "Kaydediliyor..." : "Üretimi Kaydet"}
                </button>
              </footer>
            </>
          ) : (
      ...
      {manualOpen ? (
        <div className="iw-modal-backdrop">
          <section className="iw-modal-box production-create-modal">
            <div className="iw-card-head">
              <div>
                <h2>
                  <Plus size={18} /> İrsaliyesiz Üretim İşi Aç
                </h2>
                <small>
                  Model Desen merkezinde tek kayıt olarak açılır ve diğer bütün
                  bölümlerde aynı ID kullanılır.
                </small>
              </div>
              <button
                className="iw-btn"
            ...
            <div className="iw-card-body production-create-grid">
              <label>
                <span>Model adı *</span>
                <input
                  value={manual.modelName}
                  onChange={(event) =>
                    setManual({ ...manual, modelName: event.target.value })
                  }
                />
              </label>
              <label>
                ...
                <input
                  value={manual.firma}
                  onChange={(event) =>
                    setManual({ ...manual, firma: event.target.value })
                  }
                />
              </label>
              <label>
                <span>Referans / Sipariş no</span>
                <input
                  value={manual.dispatchNo}
                  onChange={(event) =>
                    setManual({ ...manual, dispatchNo: event.target.value })
                  }
                />
              </label>
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={manual.expectedQty}
                  onChange={(event) =>
                    setManual({ ...manual, expectedQty: event.target.value })
                  }
                />
              </label>
              <label>
                ...
                <input
                  value={manual.printArea}
                  onChange={(event) =>
                    setManual({ ...manual, printArea: event.target.value })
                  }
                />
              </label>
            </div>
