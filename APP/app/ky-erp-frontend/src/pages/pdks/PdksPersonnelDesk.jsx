...
import { saveIkControlChanges } from "../../services/ikPersonnelControlApi";
...
const dateTr=(date)=>{const raw=String(date||"").trim();if(!raw)return"";const value=/^\\d{10}$/.test(raw)?new Date(Number(raw)*1000):/^\\d{13}$/.test(raw)?new Date(Number(raw)):new Date(`${raw.slice(0,10)}T12:00:00`);return Number.isNaN(value.getTime())?raw:value.toLocaleDateString("tr-TR")};
...
export default function PdksPersonnelDesk({activeTab="personel-bilgileri",activeMainCompany,isAuditAccount=false,openModule}){
  ...
  const [cardDraft,setCardDraft]=useState({cardNo:"",reason:""});
  ...
  const loadCore=useCallback(async()=>{
    ...
    try{
      const [peopleResult,configResult,leaveResult]=await Promise.allSettled([
        getPdksPeople(),
        getPdksModernConfig({mainCompanyId:company}),
        getPdksLeaveCenter({mainCompanyId:company,from:`${year}-01-01`,to:`${year}-12-31`}),
      ]);
      if(peopleResult.status!=="fulfilled") throw peopleResult.reason;
      const list=Array.isArray(peopleResult.value)?peopleResult.value:[];
      setPeople(list);
      setModern(configResult.status==="fulfilled"?(configResult.value||{leaveTypes:[]}):{leaveTypes:[]});
      setLeaveCenter(leaveResult.status==="fulfilled"?(leaveResult.value||{plans:[]}):{plans:[]});
      setSelectedId(current=>list.some(p=>p.id===current)?current:(list[0]?.id||""));
      if(configResult.status==="rejected"||leaveResult.status==="rejected"){
        setNotice("Personel ve puantaj açıldı. Yardımcı PDKS verilerinin bir bölümü alınamadı; ekran çalışmaya devam ediyor.");
      }
    }catch(cause){setError(cause?.message||"PDKS personel merkezi yüklenemedi.")}
  ...
  const loadPerson=useCallback(async()=>{
    ...
    setBusy(true);setError("");
    try{
      const [attResult,entResult]=await Promise.allSettled([
        getPdksAttendance(selected.id,year,month),
        getPdksLeaveEntitlement(selected.id,{mainCompanyId:company}),
      ]);
      if(attResult.status!=="fulfilled") throw attResult.reason;
      setAttendance(attResult.value||null);
      setEntitlement(entResult.status==="fulfilled"?(entResult.value||null):null);
      if(entResult.status==="rejected") setNotice("Puantaj yüklendi. İzin hakediş özeti şu anda alınamadı.");
    }catch(cause){setError(cause?.message||"Personel puantajı yüklenemedi.")}
    finally{setBusy(false)}
  ...
  const openCard=()=>{
    if(!selected||isAuditAccount)return;
    setCardDraft({cardNo:selected.cardNo||"",reason:""});
    setModal("card");
  };
  const saveCard=async()=>{
    if(!selected||isAuditAccount)return;
    const nextCard=String(cardDraft.cardNo||"").trim();
    if(nextCard===String(selected.cardNo||"").trim()){setNotice("Kart numarasında değişiklik yok.");setModal("");return}
    if(!cardDraft.reason.trim()){setError("Kart değişikliği nedeni zorunludur.");return}
    setBusy(true);setError("");setNotice("");
    try{
      await saveIkControlChanges(selected.id,{effectiveDate:iso(),note:cardDraft.reason,changes:{cardNo:nextCard}});
      await Promise.all([loadCore(),loadPerson()]);
      setNotice("PDKS kart numarası personelin tek ana kaydında güncellendi.");
      setModal("");
    }catch(cause){setError(cause?.message||"Kart numarası güncellenemedi.")}
    finally{setBusy(false)}
  };

  const personnelActions=[
    ["İK Personel Kartı","primary",openIk],
    ["Kart","",openCard],
  ...
  return <div className="ppd-page">
    ...
    {modal==="card"?<Modal title="PDKS Kart Numarası" subtitle="Kart numarası personelin tek ana kaydına bağlanır. Kimlik, maaş ve özlük bilgileri İK bölümünden yönetilir." onClose={()=>setModal("")}><div className="ppd-form"><label>Kart No<input value={cardDraft.cardNo} onChange={e=>setCardDraft({...cardDraft,cardNo:e.target.value})} placeholder="Örn. 00057"/></label><label className="wide">Değişiklik Nedeni *<input value={cardDraft.reason} onChange={e=>setCardDraft({...cardDraft,reason:e.target.value})} placeholder="Yeni kart, kart değişimi, kayıp kart..."/></label></div><div className="ppd-modal-actions"><button onClick={()=>setModal("")}>Vazgeç</button><button className="ppd-primary" onClick={saveCard} disabled={busy||!cardDraft.reason.trim()}>Kartı Kaydet</button></div></Modal>:null}
