...
export default function QueueDesignModal({
  ...
  async function addToQueue() {
    ...
    setBusy(false);
    if (failures.length) {
      setError(`${selectedIds.length - failures.length} iş kuyruğa eklendi. Eklenemeyenler: ${failures.join(" | ")}`);
  ...
  return <div className="bh-modal" role="dialog" aria-modal="true"><div className="bh-modal-card bh-queue-modal">
    ...
    <h2>{sampleMode ? "Desenlerden Numune İşi Aç" : "Desenlerden İmalat İşi Aç"}</h2>
    <p className="bh-queue-help">Desen Havuzu'ndaki gerçek modeller burada görünür. Seçilen model {sampleMode ? "ayrı bir numune işine" : "ayrı bir imalat boyası işine"} dönüştürülür; iki çalışma birbirine karışmaz.</p>
