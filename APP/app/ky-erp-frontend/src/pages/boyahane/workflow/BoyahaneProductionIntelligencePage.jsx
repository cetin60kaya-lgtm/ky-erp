...
function draftRows(recipe, products, lots) {
  return safeArray(recipe?.lines).map((line, index) => {
    ...
    return {
      ...
      lotId: lot?.id || "",
      fixed: index < 2 && ["S10", "S20"].some((key) => norm(product?.productName || line.productName).includes(key)),
...
function stockState(candidate, products, lots) {
  ...
  return lines.map((line) => {
    ...
    return {
      ...
      referenceGram: Number(line.referenceGram || 0),
      lot,
      remainingKg: Number(lot?.remainingKg ?? lot?.remainingQuantity ?? lot?.quantity ?? 0),
    };
  });
}
...
function ColorAssistantDock({
  ...
  }, [activeMainCompany?.slug, moduleActionContext?.boyahaneJobId, moduleActionContext?.nonce]);
  ...
  useEffect(() => {
    ...
    if (!colors.some((row) => txt(row.id) === txt(colorId))) setColorId(colors[0].id);
  }, [jobId, colors, colorId]);
  ...
  async function analyze() {
    ...
    try {
      ...
      if (!safeArray(payload?.suggestions).length) {
        setNotice("Bu renk için HEX ile karşılaştırılabilir geçmiş eşleşme bulunmadı. Yeni doğrulamalar yapıldıkça sistem otomatik genişleyecek.");
      }
    } catch (requestError) {
  ...
  async function applySuggestion() {
    ...
    try {
      ...
      const patch = {
        registeredColorId: selected.registeredColorId || color.registeredColorId || "",
        pantone: selected.pantone || color.pantone || "",
        colorHex: chosenHex,
        colorName: nextColorName,
        paintType: color.paintType || color.dyeType || selected.paintType || "SUBAZLI",
        sourceType: selected.pantone ? "PANTONE" : color.sourceType || color.colorSource || "VISUAL",
        colorSource: selected.pantone ? "PANTONE" : color.colorSource || color.sourceType || "VISUAL",
        aiColorSuggestion: {
      ...
      setError(requestError?.message || "AI önerisi çalışma taslağına aktarılamadı.");
    } finally {
      setSaving(false);
    }
  }
  ...
  function askAi() {
    ...
    const candidates = suggestions.slice(0, 3).map((row) =>
    ...
    const stockText = selected?.recipe
      ? stock.map((row) => `${row.productName}: ${row.lot ? `${row.lot.lotNo || "lot"} ${formatKg(row.remainingKg)}` : "aktif lot yok"}`).join("; ")
      : "Önerilen reçete yok";
    openModule("asistan", {
      tabKey: "sohbet",
      actionContext: {
    ...
    <section className={`bh-ai-center ${open ? "open" : "closed"}`}>
      <header className="bh-ai-head">
        <div className="bh-ai-title">
          <span className="bh-ai-icon"><Sparkles size={20} /></span>
          <div>
            <small>CANLI RENK BİLİMİ + REÇETE GEÇMİŞİ + KY ERP AI</small>
            <h2>Akıllı Renk & Pantone Merkezi</h2>
            <p>HEX’e göre en yakın kayıtlı Pantone ve reçeteyi bulur; gerçek lotu kontrol eder. Yeni doğrulamalar sistemi kalıcı olarak geliştirir.</p>
          </div>
        </div>
        <div className="bh-ai-head-actions">
          {result?.catalogStats ? <span className="bh-ai-stat">{result.catalogStats.pantones} Pantone · {result.catalogStats.recipes} reçete</span> : null}
          <button type="button" className="bh-ai-button ghost" onClick={load} disabled={loadingData}><RefreshCw size={15} /> Güncelle</button>
          <button type="button" className="bh-ai-button" onClick={() => setOpen((current) => !current)}>{open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}{open ? "Daralt" : "Aç"}</button>
        </div>
      </header>
      ...
      {open ? (
        <div className="bh-ai-body">
          <section className="bh-ai-control">
            <div className="bh-ai-section-title"><Palette size={17} /><div><b>1. Model ve hedef renk</b><span>Canlı Boyahane işinden seç veya yeni HEX gir.</span></div></div>
            <label><span>Model</span><select value={jobId} disabled={loadingData} onChange={(event) => { setJobId(event.target.value); setColorId(""); }}><option value="">Model seç</option>{jobs.map((row) => <option key={row.id} value={row.id}>{row.modelName || "Model"} · {row.companyName || "-"}</option>)}</select></label>
            <label><span>Model rengi</span><select value={color?.id || ""} disabled={!job} onChange={(event) => setColorId(event.target.value)}><option value="">Renk seç</option>{colors.map((row) => <option key={row.id} value={row.id}>{row.colorName || "Renk"} · {row.pantone || row.colorHex || "kod yok"}</option>)}</select></label>
            <div className="bh-ai-color-row">
              <input className="bh-ai-picker" type="color" value={normalizeHex(hex) || "#808080"} onChange={(event) => setHex(event.target.value.toUpperCase())} aria-label="Renk seç" />
              <label><span>HEX</span><input value={hex} placeholder="#173B77" onChange={(event) => setHex(event.target.value.toUpperCase())} /></label>
              <label><span>Pantone (opsiyonel)</span><input value={pantoneQuery} placeholder="19-4151" onChange={(event) => setPantoneQuery(event.target.value.toUpperCase())} /></label>
            </div>
            <button type="button" className="bh-ai-button primary wide" onClick={analyze} disabled={analyzing || !color}>{analyzing ? <RefreshCw className="spin" size={16} /> : <Sparkles size={16} />}{analyzing ? "Canlı kayıtlar taranıyor…" : "Tüm Kayıtlı Renkleri Tara"}</button>
            {result?.catalogStats ? (
              <div className="bh-ai-catalog">
                <span><b>{result.catalogStats.registeredColors}</b>Kayıtlı renk</span>
                <span><b>{result.catalogStats.hexMappedPantones}</b>HEX eşlemeli Pantone</span>
                <span><b>{result.catalogStats.confirmedMappings}</b>Onaylanmış eşleşme</span>
                <span><b>{result.catalogStats.productions}</b>Üretim geçmişi</span>
              </div>
            ) : null}
          </section>

          <section className="bh-ai-suggestions">
            <div className="bh-ai-section-title"><Sparkles size={17} /><div><b>2. En yakın Pantone / reçete</b><span>CIEDE2000 renk mesafesi + canlı geçmiş sıralaması.</span></div></div>
            {suggestions.length ? (
              <div className="bh-ai-candidate-list">
                {suggestions.map((row, index) => (
                  <button key={`${row.pantone}-${row.colorHex}-${index}`} type="button" className={`bh-ai-candidate ${selectedIndex === index ? "selected" : ""}`} onClick={() => setSelectedIndex(index)}>
                    <i style={{ background: row.colorHex || "#D9E2EC" }} />
                    <span className="bh-ai-candidate-main"><b>{row.pantone || "Pantone yok"} · {row.colorName || "Renk"}</b><small>{row.colorHex || "HEX kaydı yok"} · {row.paintType}</small></span>
                    <span className={`bh-ai-confidence ${confidenceClass(row.confidence)}`}><b>%{row.score}</b><small>{row.deltaE === null ? "kod eşleşmesi" : `ΔE ${row.deltaE}`}</small></span>
                    <span className="bh-ai-recipe">{row.recipe ? `${row.recipe.version} · ${row.recipe.lines.length} bileşen` : "Reçete yok"}</span>
                  </button>
                ))}
              </div>
            ) : (
              <div className="bh-ai-empty">{analyzing ? "Renkler karşılaştırılıyor…" : "Model rengini seçip analizi çalıştır."}</div>
            )}
          </section>

          <section className="bh-ai-stock">
            <div className="bh-ai-section-title"><Droplets size={17} /><div><b>3. Reçete ve gerçek lot</b><span>Öneri stok düşmeden önce lot yeterliliğini gösterir.</span></div></div>
            {selected ? (
              <>
                <div className="bh-ai-selected">
                  <i style={{ background: selected.colorHex || "#D9E2EC" }} />
                  <div><b>{selected.pantone} · {selected.colorName || "Renk"}</b><span>{selected.confirmed ? "Daha önce kullanıcı tarafından doğrulanmış" : selected.hexOrigin === "MODEL_COLOR" ? "Model renk geçmişinden" : "Kayıtlı renk kartından"}</span></div>
                </div>
                {stock.length ? <div className="bh-ai-stock-list">{stock.map((row) => <div key={row.key} className={row.lot ? "ok" : "missing"}>{row.lot ? <CheckCircle2 size={15} /> : <AlertTriangle size={15} />}<span><b>{row.productName}</b><small>{row.referenceGram.toFixed(2)} g · {row.lot ? `${row.lot.lotNo || "Lot"} · ${formatKg(row.remainingKg)}` : "Aktif lot bulunamadı"}</small></span></div>)}</div> : <div className="bh-ai-empty">Bu Pantone için bileşenli reçete geçmişi yok.</div>}
                <div className="bh-ai-actions">
                  <button type="button" className="bh-ai-button ghost" onClick={askAi} disabled={!openModule}><Bot size={16} /> KY ERP AI ile Yorumla</button>
                  <button type="button" className="bh-ai-button primary" onClick={applySuggestion} disabled={saving}>{saving ? "Kaydediliyor…" : selected.recipe ? "Onayla + Taslağa Aktar" : "Eşleşmeyi Onayla"}</button>
                </div>
              </>
      ...
      <footer className="bh-ai-foot">Pantone/HEX ekran karşılığı bir renk yakınlığıdır; fiziksel boya sonucu kumaş, baz, ürün ve prosesle değişebilir. Sistem formülü gizlice üretime yazmaz: öneri → kullanıcı onayı → taslak → gerçek lot kontrolü → üretim.</footer>
    </section>
  );
}
...
export default function BoyahaneProductionIntelligencePage({
  ...
  }, [moduleActionContext?.nonce, moduleActionContext?.boyahaneJobId]);
    ...
    <div className="bh-production-intelligence">
      <ColorAssistantDock
        ...
        onDraftApplied={({ jobId }) => {
          setSerialContext((current) => ({ ...current, boyahaneJobId: jobId, nonce: String(Date.now()) }));
