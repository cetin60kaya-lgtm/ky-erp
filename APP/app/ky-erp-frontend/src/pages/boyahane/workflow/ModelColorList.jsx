...
export default function ModelColorList({ colors, selectedId, onSelect, onAdd, onDelete }) {
  ...
  const filtered = colors.filter((row) => {
    const text = `${row.colorName || ""} ${row.pantone || ""}`.toLocaleLowerCase("tr-TR");
    return (!query || text.includes(query.toLocaleLowerCase("tr-TR"))) && (!status || row.status === status) && (!paintType || row.paintType === paintType);
  });
  return (
    <aside className="bh-card bh-color-sidebar">
      ...
      <div className="bh-card-body">
        <div className="bh-form-grid">
          <label className="bh-field"><span>Renk veya Pantone ara</span><input value={query} onChange={(event) => setQuery(event.target.value)} /></label>
          <div className="bh-form-grid two"><label className="bh-field"><span>Durum</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Tümü</option>{Object.entries(COLOR_STATUS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label className="bh-field"><span>Boya türü</span><select value={paintType} onChange={(event) => setPaintType(event.target.value)}><option value="">Tümü</option>{paintTypes.map((item) => <option key={item}>{item}</option>)}</select></label></div>
        </div>
        ...
        <div className="bh-list bh-work-color-list">
          {filtered.map((color) => (
            <button key={color.id} className={color.id === selectedId ? "active" : ""} onClick={() => onSelect(color.id)}>
              ...
              <span>{color.pantone || "Pantone yok"} • {color.paintType || "Boya türü yok"}</span>
              <span>{color.printRegion || "Baskı bölgesi yok"} • Plan {formatKg(color.plannedKg)}</span>
              <span><em className={`bh-status ${statusTone(color.status)}`}>{COLOR_STATUS[color.status] || color.status}</em></span>
            </button>
          ))}
