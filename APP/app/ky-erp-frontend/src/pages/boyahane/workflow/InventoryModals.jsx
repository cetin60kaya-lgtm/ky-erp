    ...
    <div className="bh-modal" role="dialog" aria-modal="true">
      ...
      >
        ...
        <div className="bh-form-grid two">
          ...
          <label className="bh-field"><span>LOT politikası</span><select value={form.lotPolicy} onChange={(event) => set("lotPolicy", event.target.value)}><option value="REQUIRED">LOT Zorunlu</option><option value="OPTIONAL">LOT Opsiyonel</option><option value="NONE">LOT Kullanılmaz</option></select></label>
        ...
        <div className="bh-notice warning">LOT kuralı ürün kartına aittir. Kimyasal tedarikçinin servis/gider kalemleri otomatik LOT zorunlu olmaz.</div>
...
export function StockMovementModal({ lot, busy, onCancel, onSave }) {
  ...
  const outgoingTypes = ["OUT", "PRODUCTION", "SAMPLE", "FIRE", "RETURN", "ADJUSTMENT_OUT", "CORRECTION_OUT"];

  return (
    <div className="bh-modal" role="dialog" aria-modal="true">
      <form className="bh-modal-card" onSubmit={(event) => { event.preventDefault(); onSave({ ...form, reason: form.type, quantity: Number(form.quantity || 0) }); }}>
        ...
        <div className="bh-form-grid two">
          <label className="bh-field"><span>Hareket</span><select value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value }))}><option value="PRODUCTION">Üretim Kullanımı</option><option value="SAMPLE">Numune Kullanımı</option><option value="OUT">Diğer Sarf / Çıkış</option><option value="FIRE">Fire</option><option value="RETURN">Tedarikçiye İade</option><option value="IN">Ek Giriş</option><option value="ADJUSTMENT_OUT">Sayım Eksiği</option><option value="ADJUSTMENT_IN">Sayım Fazlası</option><option value="CORRECTION_OUT">Düzeltme Çıkışı</option><option value="CORRECTION_IN">Düzeltme Girişi</option></select></label>
          <label className="bh-field"><span>Miktar</span><input required type="number" min="0.001" step="0.001" max={outgoingTypes.includes(form.type) ? remaining : undefined} value={form.quantity} onChange={(event) => setForm((current) => ({ ...current, quantity: event.target.value }))} /></label>
          ...
          <label className="bh-field"><span>Kaynak</span><select value={form.source} onChange={(event) => setForm((current) => ({ ...current, source: event.target.value }))}><option value="MANUAL">Elle hareket</option><option value="ACCOUNTING">Muhasebe</option><option value="SAMPLE">Numune</option><option value="PRODUCTION">İmalat</option><option value="COUNT">Sayım</option><option value="REVERSAL">Ters / Düzeltme</option></select></label>
          <label className="bh-field wide"><span>Açıklama</span><textarea required value={form.note} onChange={(event) => setForm((current) => ({ ...current, note: event.target.value }))} placeholder="Üretim, reçete, numune, fire, sayım, iade veya düzeltme açıklaması" /></label>
        </div>
        <div className="bh-notice warning">Çıkış hareketi kalan stoku aşamaz. Yanlış kayıt silinmez; ters hareket oluşturulur ve maliyet hareket anında saklanır.</div>
