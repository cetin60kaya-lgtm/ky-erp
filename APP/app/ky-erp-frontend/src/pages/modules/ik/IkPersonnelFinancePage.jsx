...
export default function IkPersonnelFinancePage({ activeMainCompany, focus = "personel" }) {
  ...
  const loadDetail = useCallback(async (employeeId = selectedId) => {
    if (!employeeId) { setDetail(null); setLeaveEntitlement(null); return null; }
    const [result, entitlement] = await Promise.all([
      getIkControlPerson(employeeId),
      getIkControlLeaveEntitlement(employeeId).catch(() => null),
    ]);
    setDetail(result || null);
    setLeaveEntitlement(entitlement || null);
    return result;
  }, [selectedId]);
  ...
  useEffect(() => {
    if (!selectedId) { setDetail(null); setLeaveEntitlement(null); return; }
    let cancelled = false;
    setBusy(true);
  ...
  const leaveLedgerRows = Array.isArray(leaveEntitlement?.rows) ? leaveEntitlement.rows : [];
  const leaveEarned = num(leaveEntitlement?.currentCardEntitlement ?? person?.annualLeaveEntitlement);
  const leaveCarryover = num(leaveEntitlement?.carryover ?? person?.annualLeaveCarryover);
  const leaveUsed = num(leaveEntitlement?.used);
  const leaveTotalRight = leaveEarned + leaveCarryover;
  const leaveRemaining = leaveEntitlement ? num(leaveEntitlement.remaining) : Math.max(0, leaveTotalRight - leaveUsed);
        ...
        {!selected ? <div className="ikpf-empty large"><UserRound size={42}/><h2>Personel seçin</h2></div> : <>
          ...
          {!audit ? <section className="ikpf-card emphasis">
            ...
            <div className="ikpf-form-grid">
              ...
              <Field label="Yıllık İzin Hakedişi"><input disabled={!editing} type="number" value={num(person?.annualLeaveEntitlement)} onChange={(e)=>setDraft({...draft,annualLeaveEntitlement:Number(e.target.value||0)})}/></Field>
              <Field label="Devreden Yıllık İzin"><input disabled={!editing} type="number" value={num(person?.annualLeaveCarryover)} onChange={(e)=>setDraft({...draft,annualLeaveCarryover:Number(e.target.value||0)})}/></Field>
          ...
          <section className="ikpf-card ikpf-leave-card">
            <div className="ikpf-section-title"><div><BadgeCheck size={18}/><strong>Yıllık İzin Hakediş / Bakiye</strong></div><span>İK özlük özeti · izin hareketi PDKS</span></div>
            <div className="ikpf-leave-summary">
              <div><span>Hakediş</span><strong>{leaveEarned} gün</strong><small>Personel kartı güncel hakkı</small></div>
              <div><span>Devreden</span><strong>{leaveCarryover} gün</strong><small>Önceki dönemden gelen</small></div>
              <div><span>Toplam Hak</span><strong>{leaveTotalRight} gün</strong><small>Hakediş + devreden</small></div>
              <div><span>Kullanılan</span><strong>{leaveUsed} gün</strong><small>Onaylı yıllık izinler</small></div>
              <div className="remaining"><span>Kalan</span><strong>{leaveRemaining} gün</strong><small>Güncel kullanılabilir bakiye</small></div>
            </div>
            {leaveEntitlement?.warning ? <div className="ikpf-leave-warning">{leaveEntitlement.warning}</div> : null}
            <div className="ikpf-leave-ledger-title"><strong>Hakediş Geçmişi</strong><span>İşe giriş tarihine göre otomatik kıdem hakediş takvimi</span></div>
            <div className="ikpf-table entitlement">
              <div className="head"><span>Hizmet Yılı</span><span>Hakediş Tarihi</span><span>Hakediş</span><span>Yaş</span></div>
              {leaveLedgerRows.slice().reverse().slice(0,30).map((row)=><div key={`${row.serviceYear}-${row.entitlementDate}`}><b>{row.serviceYear}. yıl</b><span>{dateOnly(row.entitlementDate)||"-"}</span><strong>{num(row.entitlementDays)} gün</strong><span>{row.age ?? "-"}</span></div>)}
              {!leaveLedgerRows.length ? <p>Henüz otomatik hakediş tarihi oluşmadı veya işe giriş tarihi eksik.</p> : null}
            </div>
          </section>
          ...
          <section className="ikpf-card subtle">
            <div className="ikpf-section-title"><div><BadgeCheck size={18}/><strong>İzin Geçmişi</strong></div><span>PDKS izin hareketlerinden okunur</span></div>
            <div className="ikpf-table leave">
              ...
              {!leaveRows.length ? <p>Kayıtlı izin hareketi yok.</p> : null}
            </div>
          </section>
        </>}
      </main>
