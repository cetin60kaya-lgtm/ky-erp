...
function ControlledEmptyState({ requestedTab, goTab }) {
  return (
    <section className="accounting-empty" role="status">
      ...
      <span>{requestedTab ? `“${requestedTab}” bağlantısı artık kullanılmıyor.` : "Geçerli bir ekran seçin."}</span>
      <button type="button" className="accounting-primary" onClick={() => goTab("yonetim-ozeti")}>Yönetim özetine dön</button>
    </section>
  );
}
