...
function DetailDrawer({ id, onClose, onChanged }) {
  ...
  return <div className="eb-drawer-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><aside className="eb-drawer">
    ...
    {!detail ? <div className="eb-loading"><LoaderCircle className="eb-spin" /> Belge yükleniyor</div> : <div className="eb-drawer-body">
      ...
      <section><div className="eb-section-title"><strong>Kalemler</strong><span>{detail.lines?.length || 0} satır</span></div>{detail.lines?.length ? <div className="eb-lines"><div className="eb-line head"><span>Ürün</span><span>Miktar</span><span>LOT</span><span>Eşleşme</span></div>{detail.lines.map((line) => <div className="eb-line" key={line.id}><span><strong>{line.description || line.product_code || "Kalem"}</strong><small>{line.product_code || line.supplier_product_code || ""}</small></span><span>{Number(line.quantity || 0).toLocaleString("tr-TR")} {line.unit_code || ""}</span><span>{line.raw_metadata?.lotNo || "-"}</span><span className={`eb-chip ${String(line.match_status || "").toLowerCase()}`}>{line.match_status || "UNMATCHED"}</span></div>)}</div> : <Empty title="Kalem bulunamadı" text="Belge kalemleri kontrol gerektiriyor." />}</section>
