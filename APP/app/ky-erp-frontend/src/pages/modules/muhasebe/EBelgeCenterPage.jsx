...
function DetailDrawer({ id, onClose, onChanged }) {
  ...
  return <div className="eb-drawer-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><aside className="eb-drawer">
    ...
    {!detail ? <div className="eb-loading"><LoaderCircle className="eb-spin" /> Belge yükleniyor</div> : <div className="eb-drawer-body">
      ...
      <section><div className="eb-section-title"><strong>Kalemler</strong><span>{detail.lines?.length || 0} satır</span></div>{detail.lines?.length ? <div className="eb-lines"><div className="eb-line head"><span>Ürün</span><span>Miktar</span><span>LOT</span><span>Eşleşme</span></div>{detail.lines.map((line) => <div className="eb-line" key={line.id}><span><strong>{line.description || line.product_code || "Kalem"}</strong><small>{line.product_code || line.supplier_product_code || ""}</small><EBelgeLineReview documentId={id} line={line} onChanged={load} /></span><span>{Number(line.quantity || 0).toLocaleString("tr-TR")} {line.unit_code || ""}</span><span>{line.raw_metadata?.lotNo || "-"}</span><span className={`eb-chip ${String(line.match_status || "").toLowerCase()}`}>{line.match_status || "UNMATCHED"}</span></div>)}</div> : <Empty title="Kalem bulunamadı" text="Belge kalemleri kontrol gerektiriyor." />}</section>
...
export default function EBelgeCenterPage({ activeMainCompany, openModule }) {
  ...
  return <section className="eb-page">
    <div className="eb-hero"><div><span className="eb-kicker">Muhasebe · e-Belge Merkezi</span><h2>Fatura, irsaliye ve belge kontrolü tek merkezde</h2><p>İşNet, manuel XML/PDF ve diğer sağlayıcılar aynı belge havuzuna gelir. Muhasebeleştirme son kullanıcı onayından sonra yapılır.</p></div><div className="eb-hero-actions"><button type="button" onClick={load} disabled={busy}>{busy ? <LoaderCircle className="eb-spin" size={17} /> : <RefreshCw size={17} />} Güncelle</button><button type="button" className="eb-primary" onClick={() => setView("upload")}><Upload size={17} /> Belge Yükle</button></div></div>
