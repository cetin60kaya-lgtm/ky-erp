...
const buildLeaveFormDraft = useCallback((employee, selectedPlan = {}) => {
  ...
  function renderEvrak() {
    return (
      <section>
        <div className="page-head"><div><h1>SGK Bordro - Evrak - Ay Sonu</h1><p>Muhasebeden gelen XLS/XLSX bordroları şirket personeliyle eşleştirilir; harici kişiler bordro ve ödemeye alınmaz.</p></div><span className={`badge ${data.sgkImport ? "green" : "orange"}`}>{data.sgkImport ? `Bordro v${data.sgkImport.versionNo}` : "Bordro bekleniyor"}</span></div>
        ...
        {!!checks.length && <div className="card"><div className="ch"><div><b>Ay Sonu Kontrol Maddeleri</b><span>Kontrol sonucu.</span></div></div><div className="tw"><table><thead><tr><th>Kontrol maddesi</th><th>Durum</th><th>Aciklama</th><th>Islem</th></tr></thead><tbody>{checks.map((item, index) => <tr key={index}><td>{item.title || item.type}</td><td><span className={`badge ${item.ok ? "green" : "orange"}`}>{item.ok ? "Tamam" : "Duzelt"}</span></td><td>{item.detail || "-"}</td><td>{item.ok ? "-" : <button className="btn" onClick={() => setNotice("Personel özlük düzeltmesi için sol menüde Personel Kartları ekranını açın.")}>Bilgi</button>}</td></tr>)}</tbody></table></div></div>}
