...
const MODULE_KEYS=["DASHBOARD","MUHASEBE","FIRMA_CARI","BELGE_ISLEM","KDV","CEK_ODEME","DESEN","IMALAT","BOYAHANE","IK","ISNET","MAIL","ASISTAN","ADMIN","RAPORLAR"];
const MODULE_LABELS={DASHBOARD:"Yönetim Merkezi",MUHASEBE:"Muhasebe",FIRMA_CARI:"Firma / Cari",BELGE_ISLEM:"Belge İşlemleri",KDV:"KDV",CEK_ODEME:"Çek / Ödeme",DESEN:"Desen",IMALAT:"İmalat",BOYAHANE:"Boyahane",IK:"İK",ISNET:"e-Belge / İşNet",MAIL:"Mail & Dosyalar",ASISTAN:"KY ERP Asistan",ADMIN:"Yönetim",RAPORLAR:"Raporlar"};
const ROLE_LABELS={SUPER_ADMIN:"Uygulama Sahibi",ADMIN:"Uygulama Sahibi",COMPANY_ADMIN:"Firma Sahibi",MUHASEBE:"Muhasebe Kullanıcısı",DESEN:"Desen Kullanıcısı",IMALAT:"İmalat Kullanıcısı",BOYAHANE:"Boyahane Kullanıcısı",IK:"İK Kullanıcısı",DENETIM:"Denetim Kullanıcısı",VIEWER:"Özel Yetkili Kullanıcı"};
const MANAGED_ROLES=["MUHASEBE","DESEN","IMALAT","BOYAHANE","IK","DENETIM","VIEWER"];
...
export default function AdminUsersPanelV2(){
  ...
  const selectedRole=roleOf(selectedUser?.role);const selectedIsOwner=OWNER_ROLES.has(selectedRole);const selectedIsCompanyOwner=selectedRole==="COMPANY_ADMIN";
  ...
  return <div className="auc2-page">
    ...
    <div className="auc2-layout">
      ...
      <main className="auc2-main">{!selectedUser?<div className="auc2-empty">Bir kullanıcı seçin veya yeni kullanıcı oluşturun.</div>:<>
        ...
        {tab==="SECURITY"&&<section className="auc2-panel"><div className="auc2-security-grid"><div className="auc2-card"><h3>Giriş Politikası & MFA</h3><p>Parola + Authenticator tüm aktif kullanıcılar için zorunludur.</p><div className="auc2-lock-note">Sadece parola ile giriş kapalıdır. Google, Microsoft veya her ikisi seçilebilir.</div><label>Giriş Yöntemi<select value={selectedIsOwner?"ANY_MFA":currentPolicy} disabled={selectedIsOwner||!isOwner} onChange={e=>savePolicy(e.target.value,currentApproval)}>{LOGIN_POLICIES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>{!selectedIsOwner&&<label className="auc2-inline-check"><input type="checkbox" checked={currentApproval} onChange={e=>savePolicy(currentPolicy,e.target.checked)}/> Yeni cihaz girişinde yönetici onayı</label>}<div className="auc2-lock-note">Authenticator anahtarı doğrudan sıfırlanamaz. Güvenli QR yenileme yalnız Uygulama Sahibi ekranındaki yeniden kimlik doğrulama akışıyla yapılır.</div></div>
