...
export default function AdminUsersPanelV2(){
  ...
  const [search,setSearch]=useState(""),[statusFilter,setStatusFilter]=useState("ACTIVE"),[busy,setBusy]=useState(false),[message,setMessage]=useState("Kullanıcı merkezi hazırlanıyor..."),[warnings,setWarnings]=useState([]);
  ...
  const filteredUsers=useMemo(()=>{const q=search.trim().toLocaleLowerCase("tr-TR");return users.filter(row=>{if(statusFilter==="ACTIVE"&&row.isActive===false)return false;if(statusFilter==="PASSIVE"&&row.isActive!==false)return false;const haystack=`${row.fullName||""} ${row.username||""} ${row.email||""} ${ROLE_LABELS[roleOf(row.role)]||""}`.toLocaleLowerCase("tr-TR");return !q||haystack.includes(q)})},[users,search,statusFilter]);
  ...
  function startCreate(){const company=defaultCompany();setCreateForm(newUserForm(company));setCreatePermissions(permissionPreset("VIEWER"));setCreateKey(value=>value+1);setCreateOpen(true);setMessage("Yeni kullanıcı tek ekranda firma, rol, yetki ve giriş güvenliğiyle birlikte oluşturulur.")}
  ...
  async function submitCreate(event){event.preventDefault();if(!createForm.mainCompanySlug)return setMessage("Hata: Kayıtlı ana firma seçilmelidir.");if(createForm.password.length<6)return setMessage("Hata: Yeni kullanıcı şifresi boş gelmelidir ama kayıt sırasında en az 6 karakter girilmelidir.");setBusy(true);try{const created=await createUserComplete({...createForm,permissions:createPermissions});setMessage(`${created.fullName||created.username} tek işlemde oluşturuldu; firma, rol, yetki ve giriş politikası hazır.`);setCreateOpen(false);await loadAll();setSelectedUserId(created.id);setTab("OVERVIEW")}catch(error){setMessage(`Hata: ${error?.message||"Kullanıcı oluşturulamadı."}`)}finally{setBusy(false)}}
  ...
  async function savePolicy(policyValue,approvalRequired){if(!selectedUser||!isOwner||selectedIsOwner)return;setBusy(true);try{const sessionSeconds=policyValue==="PASSWORD_ONLY"?28800:36000;await updateLoginSecurityPolicy(selectedUser.id,{loginPolicy:policyValue,sessionSeconds,approvalRequired});setMessage("Giriş politikası kaydedildi.");await loadAll()}catch(error){setMessage(`Hata: ${error?.message||"Giriş politikası kaydedilemedi."}`)}finally{setBusy(false)}}
  ...
  const activeCount=users.filter(row=>row.isActive!==false).length;const auditCount=users.filter(row=>roleOf(row.role)==="DENETIM").length;const verifiedCount=users.filter(row=>row.emailVerified===true).length;
  const currentPolicy=selectedPolicy.loginPolicy||"ANY_MFA";const currentApproval=Boolean(selectedPolicy.approvalRequired);

  return <div className="auc2-page">
    <div className="auc2-titlebar"><div><small>YÖNETİM / KULLANICI & YETKİ MERKEZİ</small><h1>Kullanıcı, Yetki ve Güvenlik Merkezi</h1><p>Kullanıcı kimliği, firma, yetki, MFA ve oturumlar tek merkezden yönetilir.</p></div><div className="auc2-actions"><button className="auc2-secondary" disabled={busy} onClick={loadAll}>Yenile</button><button className="auc2-primary" disabled={busy||!isOwner} onClick={startCreate}>+ Yeni Kullanıcı</button></div></div>
    <div className={`auc2-banner ${String(message).startsWith("Hata:")?"is-error":warnings.length?"is-warn":"is-ok"}`}>{message}</div>
    <div className="auc2-metrics"><div><span>Aktif Kullanıcı</span><b>{activeCount}</b></div><div><span>Denetim Profili</span><b>{auditCount}</b></div><div><span>Aktif Oturum</span><b>{sessions.length}</b></div><div><span>Doğrulanmış E-posta</span><b>{verifiedCount}</b></div></div>

    {createOpen&&<section className="auc2-create-card"><div className="auc2-section-title"><div><h2>Yeni Kullanıcı — Tek Adım</h2><p>Kullanıcı kaydı tamamlanmadan firma, rol, yetki ve giriş güvenliği birlikte seçilir.</p></div><button className="auc2-secondary" onClick={()=>setCreateOpen(false)}>Vazgeç</button></div>
    ...
    <div className="auc2-layout">
      <aside className="auc2-users"><div className="auc2-list-title"><b>Kullanıcılar</b><small>{filteredUsers.length} kayıt gösteriliyor</small></div><div className="auc2-filter-row"><input ref={searchRef} type="search" name="kyerp-admin-user-filter" autoComplete="off" data-lpignore="true" data-1p-ignore="true" spellCheck="false" placeholder="Ad, kullanıcı veya e-posta ara" value={search} onChange={handleSearchChange}/><select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="ACTIVE">Aktif</option><option value="ALL">Tümü</option><option value="PASSIVE">Pasif</option></select></div>{search&&<button className="auc2-clear" onClick={()=>{setSearch("");searchRef.current?.focus()}}>Filtreyi temizle</button>}
      ...
      <main className="auc2-main">{!selectedUser?<div className="auc2-empty">Bir kullanıcı seçin veya yeni kullanıcı oluşturun.</div>:<>
        ...
        {tab==="SECURITY"&&<section className="auc2-panel"><div className="auc2-security-grid"><div className="auc2-card"><h3>Giriş Politikası & MFA</h3><p>Parola, Authenticator ve yeni cihaz kuralı.</p><label>Giriş Yöntemi<select value={selectedIsOwner?"ANY_MFA":currentPolicy} disabled={selectedIsOwner||!isOwner} onChange={e=>savePolicy(e.target.value,currentApproval)}>{LOGIN_POLICIES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>{!selectedIsOwner&&<label className="auc2-inline-check"><input type="checkbox" checked={currentApproval} onChange={e=>savePolicy(currentPolicy,e.target.checked)}/> Yeni cihaz girişinde yönetici onayı</label>}<div className="auc2-button-row"><button onClick={()=>resetUserMfa(selectedUser.id,"GOOGLE").then(()=>{setMessage("Google Authenticator kaydı sıfırlandı.");loadAll()}).catch(error=>setMessage(`Hata: ${error?.message}`))}>Google QR Yenile</button><button onClick={()=>resetUserMfa(selectedUser.id,"MICROSOFT").then(()=>{setMessage("Microsoft Authenticator kaydı sıfırlandı.");loadAll()}).catch(error=>setMessage(`Hata: ${error?.message}`))}>Microsoft QR Yenile</button></div></div>
          ...
          <div className="auc2-card"><h3>Şifre & Oturum Güvenliği</h3><p>Şifre alanı her açılışta boştur; tarayıcıdan mevcut şifre taşınmaz.</p><div className="auc2-password-row"><input key={`reset-${selectedUser.id}`} type="password" name={`kyerp-reset-password-${selectedUser.id}`} autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Yeni şifre"/><button onClick={resetPassword}>Şifreyi Değiştir</button><button className="auc2-danger" onClick={()=>revokeAllUserSessions(selectedUser.id).then(()=>{setMessage("Tüm oturumlar sonlandırıldı.");loadAll()}).catch(error=>setMessage(`Hata: ${error?.message}`))}>Tüm Oturumları Sonlandır</button></div></div></div></section>}

        {tab==="SESSIONS"&&<section className="auc2-panel"><div className="auc2-section-title"><div><h3>Aktif Oturumlar</h3><p>Cihaz ve oturum geçmişi.</p></div></div><div className="auc2-table"><div className="auc2-table-head"><span>Cihaz</span><span>Oluşturma</span><span>Son Görülme</span><span>İşlem</span></div>{selectedSessions.length?selectedSessions.map(row=><div className="auc2-table-row" key={row.id}><span>{friendlyDevice(row)}</span><span>{dateText(row.createdAt||row.created_at)}</span><span>{dateText(row.lastSeenAt||row.last_seen_at)}</span><span><button className="auc2-danger small" onClick={()=>revokeSession(row.id).then(()=>{setMessage("Oturum kapatıldı.");loadAll()}).catch(error=>setMessage(`Hata: ${error?.message}`))}>Kapat</button></span></div>):<div className="auc2-empty-row">Aktif oturum yok.</div>}</div><div className="auc2-history-columns"><div><h3>Oturum Geçmişi</h3>{selectedHistory.slice(0,30).map((row,index)=><div className="auc2-log" key={row.id||index}><b>{row.action||row.status||"Oturum"}</b><span>{dateText(row.createdAt||row.created_at||row.updatedAt)}</span></div>)}</div><div><h3>Güvenlik Kaydı</h3>{selectedAudit.slice(0,30).map((row,index)=><div className="auc2-log" key={row.id||index}><b>{row.action||"Güvenlik"}</b><span>{dateText(row.createdAt||row.created_at)}</span></div>)}</div></div></section>}
      </>}</main>
    </div>
  </div>
}
