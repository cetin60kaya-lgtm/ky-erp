...
const MODULE_KEYS=["DASHBOARD","MUHASEBE","FIRMA_CARI","BELGE_ISLEM","KDV","CEK_ODEME","DESEN","IMALAT","BOYAHANE","IK","PDKS","ISNET","MAIL","STORAGE_ADMIN","ASISTAN","ADMIN","RAPORLAR"];
const MODULE_LABELS={DASHBOARD:"Yönetim Merkezi",MUHASEBE:"Muhasebe",FIRMA_CARI:"Firma / Cari",BELGE_ISLEM:"Belge İşlemleri",KDV:"KDV",CEK_ODEME:"Çek / Ödeme",DESEN:"Desen",IMALAT:"İmalat",BOYAHANE:"Boyahane",IK:"İK",PDKS:"PDKS",ISNET:"e-Belge / İşNet",MAIL:"Mail & Dosyalar",STORAGE_ADMIN:"Bağlantılar & Depolama",ASISTAN:"KY ERP Asistan",ADMIN:"Yönetim",RAPORLAR:"Raporlar"};
...
function PermissionMatrix({rows,onChange,disabled=false,compact=false,viewOnly=false,viewOnlyKeys=null}){
  return <div className={`auc2-permissions ${compact?"is-compact":""}`}>
    ...
    {rows.map(row=><div className="auc2-perm-row" key={row.moduleKey}>
      ...
      {["canView","canCreate","canUpdate","canDelete","canApprove"].map(field=><label key={field}><input type="checkbox" checked={Boolean(row[field])} disabled={disabled || (viewOnly && (field!=="canView" || (Array.isArray(viewOnlyKeys) && !viewOnlyKeys.includes(row.moduleKey))))} onChange={e=>onChange?.(row.moduleKey,field,e.target.checked)}/></label>)}
    </div>)}
  </div>
}
...
export default function AdminUsersPanelV2(){
  ...
  const selectedRole=roleOf(selectedUser?.role);const selectedIsOwner=OWNER_ROLES.has(selectedRole);const selectedApprovalExempt=selectedIsOwner||selectedRole==="COMPANY_ADMIN";
  ...
  function editCreatePermission(moduleKey,field,value){if(roleOf(createForm.role)==="DENETIM"&&(moduleKey!=="PDKS"||field!=="canView"))return;setCreatePermissions(rows=>rows.map(row=>row.moduleKey===moduleKey?{...row,[field]:value}:row))}
  ...
  function editPermission(moduleKey,field,value){if(!selectedUser||selectedIsOwner)return;if(selectedRole==="DENETIM"&&field!=="canView")return;setPermissions(rows=>rows.map(row=>row.moduleKey===moduleKey?{...row,[field]:value}:row))}
  async function savePermissions(){if(!selectedUser||selectedIsOwner)return;setBusy(true);try{const clean=selectedRole==="DENETIM"?permissions.map(row=>({...row,canView:row.moduleKey==="PDKS"&&row.canView,canCreate:false,canUpdate:false,canDelete:false,canApprove:false})):permissions;await updateUserPermissions(selectedUser.id,clean);await saveIkUserScope({userId:selectedUser.id,mainCompanySlug:selectedUser.mainCompanySlug,scope:selectedRole==="DENETIM"?"AUDIT":"FULL"});setPermissions(clean);setMessage(selectedRole==="DENETIM"?"DENETİM salt okunur: yalnız firma sahibinin Gör izni verdiği modüller görünür.":"Yetkiler kaydedildi.")}catch(error){setMessage(`Hata: ${error?.message||"Yetkiler kaydedilemedi."}`)}finally{setBusy(false)}}
  ...
  return <div className="auc2-page">
    ...
    {createOpen&&<section className="auc2-create-card"><div className="auc2-section-title"><div><h2>Yeni Kullanıcı — Tek Adım</h2><p>Kullanıcı kaydı tamamlanmadan firma, rol, yetki ve zorunlu MFA birlikte seçilir.</p></div><button className="auc2-secondary" onClick={()=>setCreateOpen(false)}>Vazgeç</button></div>
      <form autoComplete="off" onSubmit={submitCreate}>
        ...
        {roleOf(createForm.role)==="DENETIM"&&<div className="auc2-lock-note">DENETİM salt okunurdur. Firma sahibi yalnız PDKS / Gör yetkisini açabilir; düzenleme, silme ve finans alanları kapalıdır.</div>}
        <h3>Başlangıç Yetkileri</h3><PermissionMatrix rows={createPermissions} onChange={editCreatePermission} viewOnly={roleOf(createForm.role)==="DENETIM"} viewOnlyKeys={["PDKS"]} compact/>
