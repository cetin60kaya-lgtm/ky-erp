...
function showRelink(){const linked=Boolean(enrollmentQuery.id&&enrollmentQuery.token);relinkMode=true;els.setupPanel.classList.remove("hidden");els.enrollmentCode.closest("label")?.classList.toggle("hidden",linked);els.deviceLabel.closest("label")?.classList.toggle("hidden",linked);els.setupTitle.textContent=linked?"Bu telefonu yeniden doğrula":"Erişimi yeniden bağla";els.setupCopy.textContent=linked?"KY ERP bağlantısı hazır. Devam etmek için mevcut KY ERP şifreni bir kez gir; cihaz anahtarı bu uygulamaya güvenli şekilde yeniden yazılsın.":"Ana KY ERP → Profil → Telefon Onayı bölümünden yedek erişim kodu oluştur. Kodu ve mevcut KY ERP şifreni gir.";els.connectButton.textContent=linked?"Bağlantıyı Tamamla":"Güvenilir Cihazı Bağla";els.cancelRelinkButton.classList.remove("hidden")}
