...
const els={
  ...
  setupTitle:qs("#setupTitle"),setupCopy:qs("#setupCopy"),passwordLabel:qs("#passwordLabel"),deviceLabelWrap:qs("#deviceLabelWrap"),deviceSummary:qs("#deviceSummary"),pendingCount:qs("#pendingCount"),pendingList:qs("#pendingList"),tabCount:qs("#tabCount"),
...
async function finalizeApprovalNotification(decision){await closeApprovalNotifications();try{const ready=registration||await navigator.serviceWorker?.ready.catch?.(()=>null);ready?.active?.postMessage({type:"KYERP_SECURITY_DECISION_DONE",decision:String(decision||"")})}catch{}}
...
function versionLabel(value){const raw=String(value||"").trim();return raw.replace(/^security-/i,"")||"v2.9"}
...
function cleanEnrollmentQuery(){try{localStorage.removeItem(PENDING_ENROLL_KEY)}catch{}try{const url=new URL(location.href);url.searchParams.delete("enrollmentId");url.searchParams.delete("enrollmentToken");url.searchParams.delete("autoRelink");history.replaceState({},"",url.pathname+url.search+url.hash)}catch{}}
...
function restoreEnrollmentLink(){try{const parsed=JSON.parse(localStorage.getItem(PENDING_ENROLL_KEY)||"null");if(!parsed?.id||!parsed?.token)return{id:"",token:""};if(Date.now()-Number(parsed.savedAt||0)>15*60*1000){localStorage.removeItem(PENDING_ENROLL_KEY);return{id:"",token:""}}return{id:String(parsed.id),token:String(parsed.token)}}catch{return{id:"",token:""}}}
...
function openAndroidBrowserInstaller(){const url=new URL(location.href);url.protocol="https:";url.host="security.kyerp.net";url.pathname="/ky-guvenlik/";url.searchParams.set("install","1");url.searchParams.set("platform","android");url.searchParams.set("browser","1");const target=url.href;const fallback=encodeURIComponent(target);location.href=`intent://${url.host}${url.pathname}${url.search}#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=${fallback};end`;}
...
async function refreshPending(preloadedItems=null){
  ...
  if(items.length){
    ...
    setEmptyState("Bekleyen onay yok","Bağlantı hazır. Yeni giriş, oturum veya güvenlik isteği geldiğinde burada görünür.","✓");
...
async function repairConnection(options={}){
  ...
  try{
    ...
    const data=response?.data||{};
    await writeDevice({...device,deviceId:data.deviceId||device.deviceId,deviceToken:data.deviceToken||device.deviceToken,deviceLabel:data.deviceLabel||device.deviceLabel,securityAppVersion:data.securityAppVersion||device.securityAppVersion||"security-v2.9",refreshedAt:data.refreshedAt||new Date().toISOString()});
