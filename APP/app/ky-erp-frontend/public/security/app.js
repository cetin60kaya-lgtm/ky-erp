...
const els={
  ...
  refreshButton:qs("#refreshButton"),deviceSummary:qs("#deviceSummary"),pendingCount:qs("#pendingCount"),pendingList:qs("#pendingList"),toast:qs("#toast")
};
...
async function connectDevice(){if(busy)return;const code=String(els.enrollmentCode.value||"").toUpperCase().replace(/[^A-Z0-9]/g,"");const password=String(els.password.value||"");if(!password) return toast("Mevcut KY ERP şifreni gir.");if(!code&&!enrollmentQuery.id)return toast("Kurulum kodunu gir.");if(isIos()&&!isStandalone())return toast("iPhone/iPad’de önce Ana Ekrana Ekle, sonra KY ERP Güvenlik ikonundan aç.");
  busy=true;els.connectButton.disabled=true;els.connectButton.textContent="Güvenlik kuruluyor...";
  try{
    const permission=Notification.permission==="granted"?"granted":await Notification.requestPermission();if(permission!=="granted")throw new Error("Bildirim izni verilmedi.");
...
async function refreshPending(){try{const payload=await deviceFetch("/auth/push/device/pending");const items=Array.isArray(payload?.data?.items)?payload.data.items:[];els.pendingList.innerHTML="";els.pendingCount.textContent=String(items.length);if(items.length){els.pendingPanel.classList.remove("hidden");els.emptyPanel.classList.add("hidden");for(const item of items)els.pendingList.appendChild(approvalCard(item));if("setAppBadge" in navigator)navigator.setAppBadge(items.length).catch(()=>{})}else{els.pendingPanel.classList.add("hidden");els.emptyPanel.classList.remove("hidden");if("clearAppBadge" in navigator)navigator.clearAppBadge().catch(()=>{})}setBadge("Hazır","ok");registration?.active?.postMessage({type:"KYERP_SECURITY_REFRESH"});return items}catch(error){if(error?.code==="PUSH_DEVICE_UNAUTHORIZED"){setBadge("Cihaz bağlantısı yenilenmeli","bad");toast("Güvenli cihaz kaydı sunucuyla eşleşmiyor. Profil > Telefon Onayı bölümünden yeni kurulum kodu ile bu cihazı bir kez yenileyin.");}else{setBadge("Bağlantı hatası","bad");toast(error?.message||"Bekleyen girişler alınamadı.");}return[]}}
async function refreshState(){const device=await readDevice().catch(()=>null);if(device?.deviceId&&device?.deviceToken){els.setupPanel.classList.add("hidden");els.readyPanel.classList.remove("hidden");els.deviceSummary.textContent=`${device.deviceLabel||"KY ERP Güvenlik cihazı"} · Güvenli cihaz imzası aktif${device.localUnlockCredentialId?" · Cihaz kilidi aktif":""}`;setBadge("Hazır","ok");await refreshPending()}else{els.setupPanel.classList.remove("hidden");els.readyPanel.classList.add("hidden");els.pendingPanel.classList.add("hidden");els.emptyPanel.classList.add("hidden");setBadge("Kurulum gerekli")}}
...
els.connectButton.addEventListener("click",connectDevice);els.refreshButton.addEventListener("click",refreshPending);
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible")refreshState()});window.addEventListener("focus",refreshState);navigator.serviceWorker?.addEventListener?.("message",()=>refreshPending());
(async function boot(){document.title="KY ERP Güvenlik";els.deviceLabel.value=defaultDeviceLabel();const url=new URL(location.href);enrollmentQuery={id:String(url.searchParams.get("enrollmentId")||""),token:String(url.searchParams.get("enrollmentToken")||"")};if(enrollmentQuery.id&&enrollmentQuery.token)toast("Kurulum bağlantısı alındı. Mevcut KY ERP şifreni gir.");renderInstall();try{await ensureWorker()}catch{}await refreshState();setInterval(()=>{if(document.visibilityState==="visible")refreshPending()},8000)})();
