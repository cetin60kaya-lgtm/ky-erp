...
function saOpenDb(){return new Promise((resolve,reject)=>{const request=indexedDB.open(KY_SECURITY_DB,1);request.onupgradeneeded=()=>{if(!request.result.objectStoreNames.contains(KY_SECURITY_STORE))request.result.createObjectStore(KY_SECURITY_STORE)};request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)})}
...
function saActionCopy(item){
  const type=String(item?.actionType||"").toUpperCase();
  const map={
    OWNER_RECOVERY_QUESTIONS_UPDATE:{title:"Güvenlik sorularını değiştir",body:"Süper Yönetici hesap kurtarma sorularını kaydetmek istiyor. Bu işlemi yalnız siz başlattıysanız onaylayın."},
    SESSION_TRUST_APPROVE:{title:"Yeni oturumu güvenilir yap",body:"Bir ERP oturumunun güvenilir cihaz olarak onaylanması isteniyor."},
    ...
    ONLY_ME:{title:"Sadece Ben Kalayım",body:"Bu cihazdaki Süper Yönetici oturumu hariç tüm aktif ERP oturumları kapatılacak."},
  };
