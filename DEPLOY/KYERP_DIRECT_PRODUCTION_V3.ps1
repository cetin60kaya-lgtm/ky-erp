...
$PDKS_POLICY_FILE = Join-Path $WORKER "migrations\0052_pdks_company_policy_hierarchy.sql"
...
Write-Host "- Yalniz additive 0023, DENETIM 0024, gerekli 0022 session guard, Mail Core 0050 ve PDKS Policy 0052 uygulanabilir." -ForegroundColor Yellow
...
if (-not (Test-Path $PDKS_POLICY_FILE)) { Fail "0052 PDKS policy sema dosyasi bulunamadi: $PDKS_POLICY_FILE" }
...
Write-Host "0052 PDKS firma/grup policy additive semasi kontrol/uygulama..." -ForegroundColor Yellow
wrangler d1 execute $DB_NAME --remote --config $DB_CONFIG --file $PDKS_POLICY_FILE
Check-Exit "0052 PDKS policy additive semasi uygulanamadi. D1 yedegi korunuyor; deploy durduruldu."

Assert-Remote-Schema-Readiness
Assert-Denetime-System-User
Write-Host "D1 hedefli uyumluluk + sema + DENETIM + MAIL + PDKS POLICY: HAZIR" -ForegroundColor Green
