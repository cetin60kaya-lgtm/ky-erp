...
'@
    $assetPattern = '(?s)Write-Host ""\r?\nWrite-Host "=== 9/11 CUSTOM DOMAIN ASSET DOGRULAMA ===" -ForegroundColor Cyan\r?\n.*?if \(\$appAsset -ne \$EXPECTED_ASSET\) \{ Fail "app\.kyerp\.net yeni build assetini gostermiyor\." \}'
    $assetRegex = [regex]::new($assetPattern)
    if (-not $assetRegex.IsMatch($source)) { Fail "V3 eski iki-domain asset kontrol blogu bulunamadi." }
    $source = $assetRegex.Replace($source, $newAssetBlock, 1)
