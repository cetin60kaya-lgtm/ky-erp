...
foreach ($cmd in @("git","node","npm","npx")) {
  if (-not (Get-Command $cmd -ErrorAction SilentlyContinue)) { Fail "$cmd bulunamadi." }
}
...
if ($dirty) {
  ...
  Fail "Tracked degisiklik var. Preflight temiz kaynakta calismalidir."
}
...
foreach ($name in $MIGRATIONS) {
  $p = Join-Path $WORKER "migrations\$name"
  if (-not (Test-Path $p)) { Fail "Migration eksik: $name" }
}
foreach ($p in @(
  (Join-Path $ROOT "tools\file-hub-agent\file-hub-agent.mjs"),
  (Join-Path $ROOT "tools\file-hub-agent\accounting-archive-worker.mjs"),
  (Join-Path $ROOT "tools\file-hub-agent\start-file-hub-agent.cmd")
)) { if (-not (Test-Path $p)) { Fail "Agent dosyasi eksik: $p" } }

Write-Host ""
Write-Host "[1/6] Worker dependency + security + typecheck + tests + dry-run" -ForegroundColor Cyan
Set-Location $WORKER
npm ci
Check "Worker npm ci basarisiz."
npm audit --omit=dev --audit-level=high
Check "Worker production dependency security audit basarisiz."
npm run typecheck
Check "Worker typecheck basarisiz."
npm test
Check "Worker testleri basarisiz."
npm run build
Check "Worker dry-run build basarisiz."
...
Write-Host ""
Write-Host "[3/6] Frontend lint + tests + production build" -ForegroundColor Cyan
Set-Location $FRONTEND
npm ci
Check "Frontend npm ci basarisiz."
npm run lint
Check "Frontend lint basarisiz."
npm test
Check "Frontend testleri basarisiz."
npm run build
Check "Frontend build basarisiz."
if (-not (Test-Path (Join-Path $FRONTEND "dist\index.html"))) { Fail "Frontend dist/index.html olusmadi." }

Write-Host ""
Write-Host "[4/6] Izole yerel D1 migration provasi" -ForegroundColor Cyan
Set-Location $WORKER
...
npx wrangler d1 execute $LOCAL_DB --local --config $LOCAL_CONFIG --persist-to $PERSIST --file local-production-center.sql
...
foreach ($needle in @('"binding": "DB"','"binding": "FILES"','"binding": "AI"','"api.kyerp.net"')) {
  if (-not $configText.Contains($needle)) { Fail "wrangler.jsonc eksik binding/route: $needle" }
}
...
Write-Host ""
Write-Host "[6/6] Sonuc" -ForegroundColor Cyan
...
Write-Host "- Hedefli 0025-0031 remote D1 migration + tam D1 backup"
