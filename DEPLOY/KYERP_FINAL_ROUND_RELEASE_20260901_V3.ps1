...
# Keep Windows console output deterministic. User-facing lines below are ASCII-only.
try {
...
Set-Content -LiteralPath $LOG -Value "KY ERP FINAL ROUND RELEASE 2026-09-01 V3`r`nBaslangic: $(Get-Date -Format o)" -Encoding UTF8

Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host " KY ERP - FINAL ROUND RELEASE / CLEAN V3" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host " Mail + MFA + Session + IsNet tenant + Worker + Pages"
Write-Host " Worker is activated BEFORE standard Resend secret put"
...
try {
    ...
    # Cloudflare Wrangler 4.127+ blocks `secret put` when the latest Worker version
    # is not the active deployment. Deploy tested source first; existing secrets are preserved.
...
Write-Host "Resend API key sadece bu terminale girilir. Chat'e veya loga yazilmaz." -ForegroundColor Yellow
...
try {
    ...
    try {
        ...
        if (-not $secretResult.Ok) {
            ...
            Fail "RESEND_API_KEY standard secret put ile aktif Worker'a yazilamadi. Latest source activation daha once tamamlanmisti; ayrinti logda."
        }
