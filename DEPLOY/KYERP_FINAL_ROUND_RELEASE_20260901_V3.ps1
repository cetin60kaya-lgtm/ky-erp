...
Write-Host " KY ERP - FINAL ROUND RELEASE / 2026-09-01 V3" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
Write-Host "- Mevcut aktif oturumlar final cutover'da bir kez kapatilir" -ForegroundColor Gray
...
Write-Host "- Auth rol/modul/login-policy D1 guard" -ForegroundColor Gray
Write-Host "- D1 reset / DROP / is verisi silme YOK" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""

foreach ($required in @($DIRECT,$CUTOVER,$ISNET_BACKFILL,$ISNET_GUARD,$AUTH_GUARD,$CONFIG)) {
...
SELECT s.email AS email
 ...
 LIMIT 1;
"@
$owner = First-D1Row -Sql $ownerSql -Label "Owner e-posta kontrolu"
...
Write-Host "Canonical deploy kendi sonunda ENTER isterse bir kez ENTER'a basin; final zincir devam eder." -ForegroundColor Yellow
...
"@
$pre = First-D1Row -Sql $preSql -Label "Auth pre-cutover kontrolu"
...
Write-Host ""
Write-Host "=== 6/8 FINAL MFA / OTURUM / ISNET / AUTH D1 CUTOVER ===" -ForegroundColor Cyan
Push-Location $WORKER
try {
    ...
    wrangler d1 execute $DB --remote --config $CONFIG --file $AUTH_GUARD | Tee-Object -FilePath $LOG -Append
    Check-Exit "0029 Auth security D1 guard uygulanamadi."
} finally { Pop-Location }
...
if ([int]$verify.auth_guard_triggers -ne 6) { Fail "Auth D1 security guard triggerlari eksik: $($verify.auth_guard_triggers)/6" }
...
Write-Host "D1 MFA + auth guard + tenant + Resend secret + API: TAM" -ForegroundColor Green
...
Write-Host "MFA              : TUM KULLANICILAR ZORUNLU + D1 GUARD" -ForegroundColor Green
...
Write-Host "Auth DB guard    : 6 TRIGGER HAZIR" -ForegroundColor Green
