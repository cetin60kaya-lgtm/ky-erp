...
function Read-State {
  ...
  return [pscustomobject]@{ lastCommandId=''; lastControlWrite='' }
}
...
function Capture-Screen([string]$Path) {
  ...
  try {
    ...
    for($i=0;$i -lt 5;$i++) {
      ...
      catch { if($i -eq 4){throw}; Start-Sleep -Milliseconds 80 }
    }
  } finally {
...
function Focus-Window([string]$ProcessName,[string]$Title,[int]$Timeout=8){
  ...
  do{
    $p=Get-Process -ErrorAction SilentlyContinue | Where-Object{
      $_.MainWindowHandle -ne 0 -and ((-not $ProcessName -or $_.ProcessName -like "*$ProcessName*") -and (-not $Title -or $_.MainWindowTitle -like "*$Title*"))
...
function Visible-Windows{
  @(Get-Process -ErrorAction SilentlyContinue | Where-Object{$_.MainWindowHandle -ne 0 -and $_.MainWindowTitle} | ForEach-Object{[ordered]@{process=$_.ProcessName;pid=$_.Id;title=$_.MainWindowTitle}})
}

function Read-Control{
  if(-not(Test-Path -LiteralPath $ControlFile)){return $null}
  for($i=0;$i -lt 8;$i++){
    try{return Get-Content -LiteralPath $ControlFile -Raw -Encoding UTF8 | ConvertFrom-Json}catch{Start-Sleep -Milliseconds 80}
  }
...
function Invoke-Action($a,[string]$ResultDir,[int]$Index){
  ...
  switch($type){
    ...
    'focus'{$timeout=if($a.timeout){[int]$a.timeout}else{8};$p=Focus-Window ([string]$a.process) ([string]$a.title) $timeout;$data=@{process=$p.ProcessName;pid=$p.Id;title=$p.MainWindowTitle}}
...
function Check-SelfUpdate{
  ...
  try{
    $sep='?';if($SelfUpdateUrl.Contains('?')){$sep='&'}
    $u=$SelfUpdateUrl+$sep+'t='+[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
    ...
    if($remote -match 'KY_AGENT_VERSION=([0-9\.]+)'){
      ...
      if($remoteVersion -ne $script:Version){
        ...
        Log "Self update $script:Version -> $remoteVersion"
...
while($true){
  try{
    ...
    if($controlWrite -and $controlWrite -ne $lastControlWrite){
      $currentCmd=Read-Control
      $lastControlWrite=$controlWrite
    }elseif(-not $currentCmd){
      $currentCmd=Read-Control
    }
    ...
    if($cmd){
      ...
      if($cmd.enabled -and $targetOk -and $cmdId -and $cmdId -ne $lastId -and -not(Expired $cmd)){
        ...
        try{
          ...
          $i=0
          foreach($a in @($cmd.actions)){
            $i++
            try{$results+=Invoke-Action $a $resultDir $i}catch{$results+=[ordered]@{index=$i;type=[string]$a.type;ok=$false;error=$_.Exception.Message};throw}
          }
        ...
        $statusMessage='Komut tamamlandı.';if($status -ne 'ok'){$statusMessage='Hata: '+$errorText}
        Write-Status $status $statusMessage $cmdId;Log "Command finish $cmdId status=$status"
      }
      ...
      if($live -and ((Get-Date)-$lastLive).TotalMilliseconds -ge $LiveIntervalMs){
        try{Capture-Screen $LiveImage;$lastLive=Get-Date}catch{Log ('Live capture error: '+$_.Exception.Message)}
      }
    }
